...
''' % (alias, type, type))
	...
	gen.insert_binding_code('''
static void _BinaryData_WriteData(hg::BinaryData *data, intptr_t ptr, size_t size) { data->Write(reinterpret_cast<const void *>(ptr), size); }
static intptr_t _BinaryData_GetData(hg::BinaryData *data) { return reinterpret_cast<intptr_t>(data->GetData()); }
''')
	gen.bind_method(binary_data, 'WriteData', 'void', ['intptr_t ptr', 'size_t size'], {'route': route_lambda('_BinaryData_WriteData')})
	gen.bind_method(binary_data, 'GetData', 'intptr_t', [], {'route': route_lambda('_BinaryData_GetData')})
...
def bind_input(gen):
	...
	gen.bind_named_enum('hg::Key', [
		'KeyLShift', 'KeyRShift', 'KeyLCtrl', 'KeyRCtrl', 'KeyLAlt', 'KeyRAlt', 'KeyLWin', 'KeyRWin', 'KeyLMenu', 'KeyRMenu',
		...
		'KeyPlus', 'KeyComma', 'KeyMinus', 'KeyPeriod',
		'KeyOEM1', 'KeyOEM2', 'KeyOEM3', 'KeyOEM4', 'KeyOEM5', 'KeyOEM6', 'KeyOEM7', 'KeyOEM8',
		'KeyBrowserBack', 'KeyBrowserForward', 'KeyBrowserRefresh', 'KeyBrowserStop', 'KeyBrowserSearch', 'KeyBrowserFavorites', 'KeyBrowserHome', 'KeyVolumeMute', 'KeyVolumeDown', 'KeyVolumeUp',
		'KeyMediaNextTrack', 'KeyMediaPrevTrack', 'KeyMediaStop', 'KeyMediaPlayPause', 'KeyLaunchMail', 'KeyLaunchMediaSelect', 'KeyLaunchApp1', 'KeyLaunchApp2',
		'KeyLast'
	])
	...
	gen.bind_method(input_system, 'GetDevice', 'std::shared_ptr<hg::InputDevice>', ['const std::string &name'])
...
def bind_window_system(gen):
	...
	# hg::MonitorRoration
	gen.bind_named_enum('hg::MonitorRotation', ['Rotation_0', 'Rotation_90', 'Rotation_180', 'Rotation_270'], storage_type='uint8_t')
	...
	gen.bind_function('hg::GetMonitorSizeMM', 'hg::tVector2<int>', ['const hg::Monitor &monitor'])
	gen.bind_function('hg::GetMonitorContentScale', 'hg::tVector2<float>', ['const hg::Monitor &monitor'])
	...
	gen.bind_function('hg::NewFullscreenWindow', 'hg::Window', ['const hg::Monitor& monitor', 'int mode_index', '?hg::MonitorRotation rotation'])
	...
	gen.bind_function('hg::GetWindowContentScale', 'bool', ['const hg::Window &window', 'float &xscale', 'float &yscale'], features={'arg_out': ['xscale', 'yscale']})
...
def bind_core(gen):
	...
	gen.bind_members(shared_shader, ['std::string name', 'uint8_t surface_attributes', 'uint16_t surface_draw_state', 'uint8_t alpha_threshold'], ['proxy'])
	...
	gen.bind_named_enum('hg::ShaderVariable::Semantic', [
		...
		'ModelMatrix', 'InverseModelMatrix', 'NormalMatrix', 'PreviousModelMatrix', 'ViewMatrix', 'InverseViewMatrix', 'ModelViewMatrix', 'NormalViewMatrix', 'ProjectionMatrix', 'ViewProjectionMatrix', 'ModelViewProjectionMatrix', 'InverseViewProjectionMatrix', 'InverseViewProjectionMatrixAtOrigin', 'PreviousViewMatrix', 'InversePreviousViewMatrix',
...
static void _ShaderValue_SetMatrix4(hg::ShaderValue *m, hg::Matrix4 &o) {m->fv[0] = o.m[0][0]; m->fv[1] = o.m[1][0]; m->fv[2] = o.m[2][0]; m->fv[3] = 0;
																				m->fv[4] = o.m[0][1]; m->fv[5] = o.m[1][1]; m->fv[6] = o.m[2][1]; m->fv[7] = 0;
																				m->fv[8] = o.m[0][2]; m->fv[9] = o.m[1][2]; m->fv[10] = o.m[2][2]; m->fv[11] = 0;
																				m->fv[12] = o.m[0][3]; m->fv[13] = o.m[1][3]; m->fv[14] = o.m[2][3]; m->fv[15] = 1;}

static void _ShaderValue_GetMatrix4(hg::ShaderValue *m, hg::Matrix4 &o) {o.m[0][0] = m->fv[0];  o.m[1][0] = m->fv[1]; o.m[2][0] = m->fv[2];
																				o.m[0][1] = m->fv[4];  o.m[1][1] = m->fv[5]; o.m[2][1] = m->fv[6];
																				o.m[0][2] = m->fv[8];  o.m[1][2] = m->fv[9]; o.m[2][2] = m->fv[10];
																				o.m[0][3] = m->fv[12];  o.m[1][3] = m->fv[13]; o.m[2][3] = m->fv[14]; }
										
static void _ShaderValue_SetMatrix44(hg::ShaderValue *m, hg::Matrix44 &o) { m->fv[0] = o.m[0][0]; m->fv[1] = o.m[1][0]; m->fv[2] = o.m[2][0]; m->fv[3] = o.m[3][0];
																			m->fv[4] = o.m[0][1]; m->fv[5] = o.m[1][1]; m->fv[6] = o.m[2][1]; m->fv[7] = o.m[3][1];
																			m->fv[8] = o.m[0][2]; m->fv[9] = o.m[1][2]; m->fv[10] = o.m[2][2]; m->fv[11] = o.m[3][2];
																			m->fv[12] = o.m[0][3]; m->fv[13] = o.m[1][3]; m->fv[14] = o.m[2][3]; m->fv[15] = o.m[3][3]; }										
static void _ShaderValue_GetMatrix44(hg::ShaderValue *m, hg::Matrix44 &o) {o.m[0][0] = m->fv[0];  o.m[1][0] = m->fv[1]; o.m[2][0] = m->fv[2]; o.m[3][0] = m->fv[4];
																				o.m[0][1] = m->fv[4];  o.m[1][1] = m->fv[5]; o.m[2][1] = m->fv[6]; o.m[3][1] = m->fv[7];
																				o.m[0][2] = m->fv[8];  o.m[1][2] = m->fv[9]; o.m[2][2] = m->fv[10]; o.m[3][2] = m->fv[11];
																				o.m[0][3] = m->fv[12];  o.m[1][3] = m->fv[13]; o.m[2][3] = m->fv[14]; o.m[3][3] = m->fv[15]; }
...
''')
	...
	gen.bind_method(shader_value, 'SetMatrix4', 'void', ['hg::Matrix4 &o'], {'route': route_lambda('_ShaderValue_SetMatrix4')})	
	gen.bind_method(shader_value, 'GetMatrix4', 'void', ['hg::Matrix4 &o'], {'arg_out': ['o'], 'route': route_lambda('_ShaderValue_GetMatrix4')})
	gen.bind_method(shader_value, 'SetMatrix44', 'void', ['hg::Matrix44 &o'], {'route': route_lambda('_ShaderValue_SetMatrix44')})	
	gen.bind_method(shader_value, 'GetMatrix44', 'void', ['hg::Matrix44 &o'], {'arg_out': ['o'], 'route': route_lambda('_ShaderValue_GetMatrix44')})
	...
	gen.bind_method(shared_material, 'GetValue', 'hg::MaterialValue *', ['const std::string &name'], {'proxy': None})
	gen.bind_method(shared_material, 'AddValue', 'hg::MaterialValue *', ['const std::string &name', 'hg::ShaderType type'], ['proxy'])
	
	gen.insert_binding_code('''static size_t _Material_GetValueCount(hg::Material *mat) { return mat->values.size(); }''')
	gen.bind_method(shared_material, 'GetValueCount', 'size_t', [], {'proxy':None, 'route': route_lambda('_Material_GetValueCount')})
	gen.insert_binding_code('''static std::string _Material_GetValueName(hg::Material *mat, const size_t &idx) { return (idx < 0 || idx >= mat->values.size()) ? "" : mat->values[idx].name; }''')
	gen.bind_method(shared_material, 'GetValueName', 'const std::string &', ['const size_t &index'], {'proxy':None, 'route': route_lambda('_Material_GetValueName')})
	...
	gen.bind_method(shared_geometry, 'GetPolygonBinding', 'std::vector<uint32_t>', ['uint32_t index'], ['proxy'])
...
''')
	...
	bind_std_vector(gen, shared_geometry)

	gen.bind_function('GeometrySmoothRGB', 'void', ['hg::Geometry &geo', 'uint32_t pass_count', 'float max_smoothing_angle'])

	#
	gen.add_include('engine/geometry_merge.h')

	gen.insert_binding_code('''static bool _Geometry_Merge(const std::string &name, const std::vector<std::shared_ptr<hg::Geometry>> &geos, const std::vector<hg::Matrix4> &mtxs, std::shared_ptr<hg::Geometry> &out) { 
	if (geos.size() != mtxs.size())
		return false;

	size_t count = geos.size();

	std::vector<hg::GeometryMergeEntry> entries(count);
	for (size_t i = 0; i < count; ++i)
		entries[i] = {geos[i], mtxs[i]};

	out = hg::GeometryMerge(name, entries);
	return true;
}
''')

	protos = [('bool', ['const std::string &name', 'const std::vector<std::shared_ptr<hg::Geometry>> &geos', 'const std::vector<hg::Matrix4> &mtxs', 'std::shared_ptr<hg::Geometry> &out'], {'arg_out': ['out'], 'route': route_lambda('_Geometry_Merge')})]
	gen.bind_function_overloads('GeometryMerge', expand_std_vector_proto(gen, protos))
...
''')
	...
	gen.bind_method(vtx_layout, 'GetAttribute', 'const hg::VertexLayout::Attribute*', ['hg::VertexAttribute::Semantic semantic'])
...
def bind_frame_renderer(gen):
	...
	gen.bind_method(shared_frame_renderer, 'Set', 'bool', ["const std::string &name", "const std::string &value"], ['proxy'])
	gen.bind_method(shared_frame_renderer, 'Get', 'std::string', ["const std::string &name"], ['proxy'])
...
static std::shared_ptr<hg::FrameRenderer> CreateFrameRenderer() { return hg::g_frame_renderer_factory.get().Instantiate(); }
	...
	gen.bind_function('CreateFrameRenderer', 'std::shared_ptr<hg::FrameRenderer>', ['?const std::string &name'], 'GetFrameRenderer')
...
def bind_navigation(gen, shared_scene_system, shared_component):
	# hg::NavigationPath
...
#	gen.bind_method(navigation_geometry, 'GetLayers', 'const std::vector<Layer> &', [])
	...
	decl_get_set_method(gen, shared_navigation, 'std::shared_ptr<hg::NavigationGeometry>', 'Geometry', 'geometry', ['proxy'])
...
def bind_scene(gen):
	...
	def Component_rval_transform(gen, conv, expr, var_out, ownership):
		src = 'auto comp_type = (*(%s))->GetConcreteType();\n' % expr
		src += '''
static hg::Type
	*box_collision_type = hg::g_type_registry.get().GetType<hg::BoxCollision>(),
	*camera_type = hg::g_type_registry.get().GetType<hg::Camera>(),
	*capsule_collision_type = hg::g_type_registry.get().GetType<hg::CapsuleCollision>(),
	*convex_collision_type = hg::g_type_registry.get().GetType<hg::ConvexCollision>(),
	*d6_joint_type = hg::g_type_registry.get().GetType<hg::D6Joint>(),
	*decorator_type = hg::g_type_registry.get().GetType<hg::Decorator>(),
	*environment_type = hg::g_type_registry.get().GetType<hg::Environment>(),
	*instance_type = hg::g_type_registry.get().GetType<hg::Instance>(),
	*light_type = hg::g_type_registry.get().GetType<hg::Light>(),
	*mesh_collision_type = hg::g_type_registry.get().GetType<hg::MeshCollision>(),
	*navigation_type = hg::g_type_registry.get().GetType<hg::Navigation>(),
	*object_type = hg::g_type_registry.get().GetType<hg::Object>(),
	*rigid_body_type = hg::g_type_registry.get().GetType<hg::RigidBody>(),
	*script_type = hg::g_type_registry.get().GetType<hg::Script>(),
	*simple_graphic_scene_overlay_type = hg::g_type_registry.get().GetType<hg::SimpleGraphicSceneOverlay>(),
	*sphere_collision_type = hg::g_type_registry.get().GetType<hg::SphereCollision>(),
	*target_type = hg::g_type_registry.get().GetType<hg::Target>(),
	*terrain_type = hg::g_type_registry.get().GetType<hg::Terrain>(),
	*transform_type = hg::g_type_registry.get().GetType<hg::Transform>(),
    *sharpen_post_process_type = hg::g_type_registry.get().GetType<hg::SharpenPostProcess>(),
    *radial_blur_post_process_type = hg::g_type_registry.get().GetType<hg::RadialBlurPostProcess>(),
    *chromatic_dispersion_post_process_type = hg::g_type_registry.get().GetType<hg::ChromaticDispersionPostProcess>(),
    *hsl_post_process_type = hg::g_type_registry.get().GetType<hg::HSLPostProcess>(),
    *motion_blur_post_process_type = hg::g_type_registry.get().GetType<hg::MotionBlurPostProcess>(),
    *sao_post_process_type = hg::g_type_registry.get().GetType<hg::SAOPostProcess>(),
	*bloom_post_process_type = hg::g_type_registry.get().GetType<hg::BloomPostProcess>();
\n'''
		src += 'if (comp_type == box_collision_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::BoxCollision>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::BoxCollision> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == camera_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::Camera>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::Camera> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == capsule_collision_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::CapsuleCollision>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::CapsuleCollision> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == convex_collision_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::ConvexCollision>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::ConvexCollision> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == d6_joint_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::D6Joint>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::D6Joint> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == decorator_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::Decorator>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::Decorator> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == environment_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::Environment>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::Environment> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == instance_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::Instance>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::Instance> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == light_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::Light>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::Light> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == mesh_collision_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::MeshCollision>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::MeshCollision> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == navigation_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::Navigation>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::Navigation> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == object_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::Object>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::Object> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == rigid_body_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::RigidBody>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::RigidBody> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == script_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::Script>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::Script> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == simple_graphic_scene_overlay_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::SimpleGraphicSceneOverlay>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::SimpleGraphicSceneOverlay> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == sphere_collision_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::SphereCollision>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::SphereCollision> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == target_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::Target>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::Target> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == terrain_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::Terrain>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::Terrain> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == transform_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::Transform>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::Transform> *>(%s)' % expr, ownership)	
		src += 'else if (comp_type == sharpen_post_process_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::SharpenPostProcess>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::SharpenPostProcess> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == radial_blur_post_process_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::RadialBlurPostProcess>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::RadialBlurPostProcess> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == chromatic_dispersion_post_process_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::ChromaticDispersionPostProcess>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::ChromaticDispersionPostProcess> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == hsl_post_process_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::HSLPostProcess>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::HSLPostProcess> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == motion_blur_post_process_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::MotionBlurPostProcess>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::MotionBlurPostProcess> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == sao_post_process_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::SAOPostProcess>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::SAOPostProcess> *>(%s)' % expr, ownership)
		src += 'else if (comp_type == bloom_post_process_type)\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::BloomPostProcess>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::BloomPostProcess> *>(%s)' % expr, ownership)
		src += 'else\n'
	...
	shared_component.add_feature('rval_transform', Component_rval_transform)

	gen.bind_method(shared_component, 'GetSceneSystem', 'std::shared_ptr<hg::SceneSystem>', [], {'proxy': None})
	gen.bind_method(shared_component, 'GetScene', 'std::shared_ptr<hg::Scene>', [], {'proxy': None})
	gen.bind_method(shared_component, 'GetNode', 'std::shared_ptr<hg::Node>', [], {'proxy': None})
	...
	gen.bind_method(shared_component, 'GetName', 'const std::string &', [], ['proxy'])
	...
	gen.insert_binding_code('static std::shared_ptr<hg::Group> _InstanceGetGroup(hg::Instance *instance) { return instance->group; }')

	gen.bind_constructor(shared_instance, [], ['proxy'])
	decl_get_set_method(gen, shared_instance, 'std::string', 'Path', 'path', ['proxy'])
	gen.bind_method(shared_instance, 'GetState', 'hg::ComponentState', [], ['proxy'])
	gen.bind_method(shared_instance, 'GetGroup', 'std::shared_ptr<hg::Group>', [], {'proxy': None, 'route': route_lambda('_InstanceGetGroup')})

	gen.end_class(shared_instance)
	
	# hg::Metadata
	gen.add_include('engine/metadata.h')

	metadata = gen.begin_class('hg::Metadata', bound_name='Metadata_nobind', noncopyable=True, nobind=True)
	gen.end_class(metadata)

	shared_metadata = gen.begin_class('std::shared_ptr<hg::Metadata>', bound_name='Metadata', features={'proxy': lib.stl.SharedPtrProxyFeature(metadata)})
	gen.add_base(shared_metadata, shared_component)

	gen.bind_constructor(shared_metadata, [], ['proxy'])
	decl_get_set_method(gen, shared_metadata, 'std::string', 'Data', 'data', ['proxy'])

	gen.end_class(shared_metadata)
	...
	decl_get_set_method(gen, shared_target, 'std::shared_ptr<hg::Node>', 'Target', 'target', ['proxy'])
	...
	decl_get_set_method(gen, shared_environment, 'float', 'TimeOfDay', 'time_of_day', ['proxy'])

	decl_get_set_method(gen, shared_environment, 'bool', 'ClearBackgroundColor', 'clear_bg_color', ['proxy'])
	decl_get_set_method(gen, shared_environment, 'bool', 'ClearBackgroundDepth', 'clear_bg_depth', ['proxy'])
	decl_get_set_method(gen, shared_environment, 'hg::Color', 'BackgroundColor', 'background_color', ['proxy'])

	decl_get_set_method(gen, shared_environment, 'float', 'AmbientIntensity', 'ambient_intensity', ['proxy'])
	decl_get_set_method(gen, shared_environment, 'hg::Color', 'AmbientColor', 'ambient_color', ['proxy'])

	decl_get_set_method(gen, shared_environment, 'hg::Color', 'FogColor', 'fog_color', ['proxy'])
	decl_get_set_method(gen, shared_environment, 'float', 'FogNear', 'fog_near', ['proxy'])
	decl_get_set_method(gen, shared_environment, 'float', 'FogFar', 'fog_far', ['proxy'])
	...
	gen.insert_binding_code('''
static void _SimpleGraphicSceneOverlay_Lines(hg::SimpleGraphicSceneOverlay *overlay, const std::vector<hg::Vector3> &points, const std::vector<hg::Color> &colors) {
	if (points.size() != colors.size()) {
		hg::debug("Mismatch between the number of points and colors");
		return;
	}
	...
	for (size_t i = 1; i < count; ++i) {
		auto &p_a = points[i - 1], &p_b = points[i];
		auto &c_a = colors[i - 1], &c_b = colors[i];
		overlay->Line(p_a.x, p_a.y, p_a.z, p_b.x, p_b.y, p_b.z, c_a, c_b);
	}
}
''')
	gen.bind_method(shared_simple_graphic_scene_overlay, 'Lines', 'void', ['const std::vector<hg::Vector3> & points', 'const std::vector<hg::Color> & colors'], {'proxy': None, 'route': route_lambda('_SimpleGraphicSceneOverlay_Lines')})
...
''')
	...
	gen.insert_binding_code('''
static void _SimpleGraphicSceneOverlay_Quads(hg::SimpleGraphicSceneOverlay *overlay, const std::vector<hg::Matrix4> &mats, const float &width, const float &height, std::shared_ptr<hg::Texture> texture, const hg::Color &color) {
	for (auto &m : mats) {
		auto pos = m.GetTranslation();
		auto axis_x = m.GetX() * width / 2;
		auto axis_y = m.GetY() * height / 2;
...
''')
	gen.bind_method(shared_simple_graphic_scene_overlay, 'Quads', 'void', ['const std::vector<hg::Matrix4> &mats', 'const float &width', 'const float &height', 'std::shared_ptr<hg::Texture> texture', 'const hg::Color &color'], {'proxy': None, 'route': route_lambda('_SimpleGraphicSceneOverlay_Quads')})
	...
	decl_get_set_method(gen, shared_transform, 'hg::Vector3', 'Position', 'position', ['proxy'])
	decl_get_set_method(gen, shared_transform, 'hg::Vector3', 'Rotation', 'rotation', ['proxy'])
	decl_get_set_method(gen, shared_transform, 'hg::Vector3', 'Scale', 'scale', ['proxy'])
	...
	decl_get_set_method(gen, shared_cam, 'float', 'ZoomFactor', 'zoom_factor', ['proxy'])
	decl_get_set_method(gen, shared_cam, 'float', 'ZNear', 'z_near', ['proxy'])
	decl_get_set_method(gen, shared_cam, 'float', 'ZFar', 'z_far', ['proxy'])

	decl_get_set_method(gen, shared_cam, 'bool', 'Orthographic', 'is_orthographic', ['proxy'])
	decl_get_set_method(gen, shared_cam, 'float', 'OrthographicSize', 'orthographic_size', ['proxy'])
	...
	decl_get_set_method(gen, shared_obj, 'std::shared_ptr<hg::RenderGeometry>', 'Geometry', 'geometry', ['proxy'])
	...
	decl_get_set_method(gen, shared_light, 'hg::Light::Model', 'Model', ' model', ['proxy'])
	decl_get_set_method(gen, shared_light, 'hg::Light::Shadow', 'Shadow', ' shadow', ['proxy'])
	decl_get_set_method(gen, shared_light, 'bool', 'ShadowCastAll', ' shadow_cast_all', ['proxy'])
	decl_get_set_method(gen, shared_light, 'float', 'ShadowRange', ' shadow_range', ['proxy'])
	decl_get_set_method(gen, shared_light, 'float', 'ShadowBias', ' shadow_bias', ['proxy'])
	decl_get_set_method(gen, shared_light, 'hg::Vector4', 'ShadowSplit', ' shadow_split', ['proxy'])
	decl_get_set_method(gen, shared_light, 'float', 'ZNear', ' z_near', ['proxy'])

	decl_get_set_method(gen, shared_light, 'float', 'Range', ' range', ['proxy'])
	decl_get_set_method(gen, shared_light, 'float', 'ClipDistance', ' clip_distance', ['proxy'])

	decl_get_set_method(gen, shared_light, 'hg::Color', 'DiffuseColor', ' diffuse_color', ['proxy'])
	decl_get_set_method(gen, shared_light, 'hg::Color', 'SpecularColor', ' specular_color', ['proxy'])
	decl_get_set_method(gen, shared_light, 'hg::Color', 'ShadowColor', ' shadow_color', ['proxy'])

	decl_get_set_method(gen, shared_light, 'float', 'DiffuseIntensity', ' diffuse_intensity', ['proxy'])
	decl_get_set_method(gen, shared_light, 'float', 'SpecularIntensity', ' specular_intensity', ['proxy'])
	decl_get_set_method(gen, shared_light, 'float', 'ConeAngle', ' cone_angle', ['proxy'])
	decl_get_set_method(gen, shared_light, 'float', 'EdgeAngle', ' edge_angle', ['proxy'])

	decl_get_set_method(gen, shared_light, 'std::shared_ptr<hg::Texture>', 'ProjectionTexture', ' projection_texture', ['proxy'])

	gen.end_class(shared_light)

	# hg::Decorator
	gen.add_include('engine/decorator.h')

	decorator = gen.begin_class('hg::Decorator', bound_name='Decorator_nobind', noncopyable=True, nobind=True)
	gen.end_class(decorator)
	
	shared_decorator = gen.begin_class('std::shared_ptr<hg::Decorator>', bound_name='Decorator', features={'proxy': lib.stl.SharedPtrProxyFeature(decorator)})
	gen.add_base(shared_decorator, shared_component)

	gen.bind_constructor(shared_decorator, [], ['proxy'])

	decl_get_set_method(gen, shared_decorator, 'std::shared_ptr<hg::RenderGeometry>', 'Geometry', 'geo', ['proxy'])

	decl_get_set_method(gen, shared_decorator, 'float', 'Range', 'range', ['proxy'])
	decl_get_set_method(gen, shared_decorator, 'bool', 'Decimate', 'decimate', ['proxy'])
	decl_get_set_method(gen, shared_decorator, 'float', 'DecimationRangeStart', 'range', ['proxy'])

	gen.bind_method(shared_decorator, 'AddInstance', 'void', ['const hg::Matrix4 &world'], ['proxy'])

	gen.bind_method(shared_decorator, 'LoadData', 'bool', ['const std::string &path'], ['proxy'])
	gen.bind_method(shared_decorator, 'SaveData', 'bool', ['const std::string &path'], ['proxy'])

	gen.end_class(shared_decorator)	
	...
	decl_get_set_method(gen, shared_rigid_body, 'bool', 'IsSleeping', 'is_sleeping', ['proxy'])
	...
	decl_get_set_method(gen, shared_rigid_body, 'hg::Vector3', 'LinearVelocity', 'V', ['proxy'])
	decl_get_set_method(gen, shared_rigid_body, 'hg::Vector3', 'AngularVelocity', 'W', ['proxy'])

	decl_get_set_method(gen, shared_rigid_body, 'float', 'LinearDamping', 'damping', ['proxy'])
	decl_get_set_method(gen, shared_rigid_body, 'float', 'AngularDamping', 'damping', ['proxy'])

	decl_get_set_method(gen, shared_rigid_body, 'float', 'StaticFriction', 'sF', ['proxy'])
	decl_get_set_method(gen, shared_rigid_body, 'float', 'DynamicFriction', 'dF', ['proxy'])

	decl_get_set_method(gen, shared_rigid_body, 'float', 'Restitution', 'restitution', ['proxy'])

	decl_get_set_method(gen, shared_rigid_body, 'hg::RigidBodyType', 'Type', 'type', ['proxy'])

	decl_get_set_method(gen, shared_rigid_body, 'int', 'AxisLock', 'axis_lock', ['proxy'])
	decl_get_set_method(gen, shared_rigid_body, 'int', 'CollisionLayer', 'layer', ['proxy'])
	...
	decl_get_set_method(gen, shared_collision, 'float', 'Mass', 'mass', ['proxy'])
	decl_get_set_method(gen, shared_collision, 'hg::Matrix4', 'Matrix', 'matrix', ['proxy'])
	...
	decl_get_set_method(gen, shared_box_collision, 'hg::Vector3', 'Dimensions', 'dimensions', ['proxy'])
	...
	decl_get_set_method(gen, shared_mesh_collision, 'std::shared_ptr<hg::Geometry>', 'Geometry', 'geometry', ['proxy'])
	...
	decl_get_set_method(gen, shared_sphere_collision, 'float', 'Radius', 'radius', ['proxy'])
	...
	decl_get_set_method(gen, shared_capsule_collision, 'float', 'Length', 'length', ['proxy'])
	decl_get_set_method(gen, shared_capsule_collision, 'float', 'Radius', 'radius', ['proxy'])
	...
	decl_get_set_method(gen, shared_convex_collision, 'std::shared_ptr<hg::Geometry>', 'Geometry', 'geometry', ['proxy'])
	...
	decl_get_set_method(gen, shared_joint, 'hg::Vector3', 'Pivot', 'pivot', ['proxy'])
	decl_get_set_method(gen, shared_joint, 'std::shared_ptr<hg::Node>', 'OtherBody', 'other_body', ['proxy'])
	decl_get_set_method(gen, shared_joint, 'hg::Vector3', 'OtherPivot', 'other_pivot', ['proxy'])
	...
	decl_get_set_method(gen, shared_d6_joint, 'uint8_t', 'AxisLock', 'axis_lock', ['proxy'])
	decl_get_set_method(gen, shared_d6_joint, 'hg::JointLimit', 'XAxisLimit', 'x_axis_limit', ['proxy'])
	decl_get_set_method(gen, shared_d6_joint, 'hg::JointLimit', 'YAxisLimit', 'y_axis_limit', ['proxy'])
	decl_get_set_method(gen, shared_d6_joint, 'hg::JointLimit', 'ZAxisLimit', 'z_axis_limit', ['proxy'])
	decl_get_set_method(gen, shared_d6_joint, 'hg::JointLimit', 'RotXAxisLimit', 'rot_x_axis_limit', ['proxy'])
	decl_get_set_method(gen, shared_d6_joint, 'hg::JointLimit', 'RotYAxisLimit', 'rot_y_axis_limit', ['proxy'])
	decl_get_set_method(gen, shared_d6_joint, 'hg::JointLimit', 'RotZAxisLimit', 'rot_z_axis_limit', ['proxy'])
	...
	decl_get_set_method(gen, shared_terrain, 'std::string', 'Heightmap', 'heightmap', ['proxy'])
	decl_get_set_method(gen, shared_terrain, 'hg::tVector2<int>', 'HeightmapResolution', 'heightmap_resolution', ['proxy'])
	decl_get_set_method(gen, shared_terrain, 'uint32_t', 'HeightmapBpp', 'heightmap_bpp', ['proxy'])

	decl_get_set_method(gen, shared_terrain, 'hg::Vector3', 'Size', 'size', ['proxy'])

	decl_get_set_method(gen, shared_terrain, 'std::string', 'SurfaceShader', 'surface_shader', ['proxy'])

	decl_get_set_method(gen, shared_terrain, 'int', 'PatchSubdivisionThreshold', 'patch_subdv_threshold', ['proxy'])
	decl_get_set_method(gen, shared_terrain, 'int', 'MaxRecursion', 'max_recursion', ['proxy'])
	decl_get_set_method(gen, shared_terrain, 'float', 'MinPrecision', 'min_precision', ['proxy'])

	decl_get_set_method(gen, shared_terrain, 'bool', 'Wireframe', 'wireframe', ['proxy'])

	gen.bind_method(shared_terrain, 'GetTerrainSurfaceShader', 'std::shared_ptr<hg::SurfaceShader>', [], ['proxy'])
	gen.end_class(shared_terrain)

    # Post process
	gen.add_include('engine/post_process.h')

    # hg::PostProcessComponent
	post_process_component = gen.begin_class('hg::PostProcessComponent', bound_name='PostProcessComponent_nobind', noncopyable=True, nobind=True)
	gen.end_class(post_process_component)

	shared_post_process_component = gen.begin_class('std::shared_ptr<hg::PostProcessComponent>', bound_name='PostProcessComponent', features={'proxy': lib.stl.SharedPtrProxyFeature(post_process_component)})
	gen.add_base(shared_post_process_component, shared_component)
	gen.end_class(shared_post_process_component)

    # hg::SharpenPostProcess
	sharpen_post_process = gen.begin_class('hg::SharpenPostProcess', bound_name='SharpenPostProcess_nobind', noncopyable=True, nobind=True)
	gen.end_class(sharpen_post_process)

	shared_sharpen_post_process = gen.begin_class('std::shared_ptr<hg::SharpenPostProcess>', bound_name='SharpenPostProcess', features={'proxy': lib.stl.SharedPtrProxyFeature(sharpen_post_process)})
	gen.add_base(shared_sharpen_post_process, shared_post_process_component)
	gen.bind_constructor(shared_sharpen_post_process, [], ['proxy'])
	gen.bind_method(shared_sharpen_post_process, 'GetStrength', 'float', [], ['proxy'])
	gen.bind_method(shared_sharpen_post_process, 'SetStrength', 'void', ['float strength'], ['proxy'])
	gen.end_class(shared_sharpen_post_process)
	
    # hg::RadialBlurPostProcess
	radial_blur_post_process = gen.begin_class('hg::RadialBlurPostProcess', bound_name='RadialBlurPostProcess_nobind', noncopyable=True, nobind=True)
	gen.end_class(radial_blur_post_process)
	
	shared_radial_blur_post_process = gen.begin_class('std::shared_ptr<hg::RadialBlurPostProcess>', bound_name='RadialBlurPostProcess', features={'proxy': lib.stl.SharedPtrProxyFeature(radial_blur_post_process)})
	gen.add_base(shared_radial_blur_post_process, shared_post_process_component)
	gen.bind_constructor(shared_radial_blur_post_process, [], ['proxy'])
	gen.bind_method(shared_radial_blur_post_process, 'GetStrength', 'float', [], ['proxy'])
	gen.bind_method(shared_radial_blur_post_process, 'SetStrength', 'void', ['float strength'], ['proxy'])
	gen.bind_method(shared_radial_blur_post_process, 'GetCenter', 'hg::tVector2<float>', [], ['proxy'])
	gen.bind_method(shared_radial_blur_post_process, 'SetCenter', 'void', ['const hg::tVector2<float> &center'], ['proxy'])
	gen.end_class(shared_radial_blur_post_process)

    # hg::ChromaticDispersionPostProcess
	chromatic_dispersion_post_process = gen.begin_class('hg::ChromaticDispersionPostProcess', bound_name='ChromaticDispersionPostProcess_nobind', noncopyable=True, nobind=True)
	gen.end_class(chromatic_dispersion_post_process)
	
	shared_chromatic_dispersion_post_process = gen.begin_class('std::shared_ptr<hg::ChromaticDispersionPostProcess>', bound_name='ChromaticDispersionPostProcess', features={'proxy': lib.stl.SharedPtrProxyFeature(chromatic_dispersion_post_process)})
	gen.add_base(shared_chromatic_dispersion_post_process, shared_post_process_component)
	gen.bind_constructor(shared_chromatic_dispersion_post_process, [], ['proxy'])
	gen.bind_method(shared_chromatic_dispersion_post_process, 'GetWidth', 'float', [], ['proxy'])
	gen.bind_method(shared_chromatic_dispersion_post_process, 'SetWidth', 'void', ['float width'], ['proxy'])
	gen.end_class(shared_chromatic_dispersion_post_process)
	
	# hg::HSLPostProcess()
	hsl_post_process = gen.begin_class('hg::HSLPostProcess', bound_name='HSLPostProcess_nobind', noncopyable=True, nobind=True)
	gen.end_class(hsl_post_process)
	
	shared_hsl_post_process = gen.begin_class('std::shared_ptr<hg::HSLPostProcess>', bound_name='HSLPostProcess', features={'proxy': lib.stl.SharedPtrProxyFeature(hsl_post_process)})
	gen.add_base(shared_hsl_post_process, shared_post_process_component)
	gen.bind_constructor(shared_hsl_post_process, [], ['proxy'])
	gen.bind_method(shared_hsl_post_process, 'GetH', 'float', [], ['proxy'])
	gen.bind_method(shared_hsl_post_process, 'GetS', 'float', [], ['proxy'])
	gen.bind_method(shared_hsl_post_process, 'GetL', 'float', [], ['proxy'])
	gen.bind_method(shared_hsl_post_process, 'SetH', 'void', ['float h'], ['proxy'])
	gen.bind_method(shared_hsl_post_process, 'SetS', 'void', ['float s'], ['proxy'])
	gen.bind_method(shared_hsl_post_process, 'SetL', 'void', ['float l'], ['proxy'])
	gen.end_class(shared_hsl_post_process)
	
	# hg::MotionBlurPostProcess
	motion_blur_post_process = gen.begin_class('hg::MotionBlurPostProcess', bound_name='MotionBlurPostProcess_nobind', noncopyable=True, nobind=True)
	gen.end_class(motion_blur_post_process)
	
	shared_motion_blur_post_process = gen.begin_class('std::shared_ptr<hg::MotionBlurPostProcess>', bound_name='MotionBlurPostProcess', features={'proxy': lib.stl.SharedPtrProxyFeature(motion_blur_post_process)})
	gen.add_base(shared_motion_blur_post_process, shared_post_process_component)
	gen.bind_constructor(shared_motion_blur_post_process, [], ['proxy'])
	gen.bind_method(shared_motion_blur_post_process, 'GetBlurRadius', 'int', [], ['proxy'])
	gen.bind_method(shared_motion_blur_post_process, 'SetBlurRadius', 'void', ['int radius'], ['proxy'])
	gen.bind_method(shared_motion_blur_post_process, 'GetSampleCount', 'int', [], ['proxy'])
	gen.bind_method(shared_motion_blur_post_process, 'SetSampleCount', 'void', ['int count'], ['proxy'])
	gen.bind_method(shared_motion_blur_post_process, 'GetExposure', 'float', [], ['proxy'])
	gen.bind_method(shared_motion_blur_post_process, 'SetExposure', 'void', ['float exposure'], ['proxy'])
	gen.end_class(shared_motion_blur_post_process)

    # hg::SAOPostProcess
	sao_post_process = gen.begin_class('hg::SAOPostProcess', bound_name='SAOPostProcess_nobind', noncopyable=True, nobind=True)
	gen.end_class(sao_post_process)
	
	shared_sao_post_process = gen.begin_class('std::shared_ptr<hg::SAOPostProcess>', bound_name='SAOPostProcess', features={'proxy': lib.stl.SharedPtrProxyFeature(sao_post_process)})
	gen.add_base(shared_sao_post_process, shared_post_process_component)
	gen.bind_constructor(shared_sao_post_process, [], ['proxy'])
	gen.bind_method(shared_sao_post_process, 'GetBias', 'float', [], ['proxy'])
	gen.bind_method(shared_sao_post_process, 'SetBias', 'void', ['float bias'], ['proxy'])
	gen.bind_method(shared_sao_post_process, 'GetRadius', 'float', [], ['proxy'])
	gen.bind_method(shared_sao_post_process, 'SetRadius', 'void', ['float radius'], ['proxy'])
	gen.bind_method(shared_sao_post_process, 'GetQuality', 'int', [], ['proxy'])
	gen.bind_method(shared_sao_post_process, 'SetQuality', 'void', ['int quality'], ['proxy'])
	gen.bind_method(shared_sao_post_process, 'GetSampleCount', 'int', [], ['proxy'])
	gen.bind_method(shared_sao_post_process, 'SetSampleCount', 'void', ['int sample_count'], ['proxy'])
	gen.bind_method(shared_sao_post_process, 'GetSharpness', 'float', [], ['proxy'])
	gen.bind_method(shared_sao_post_process, 'SetSharpness', 'void', ['float sharpness'], ['proxy'])
	gen.bind_method(shared_sao_post_process, 'GetStrength', 'float', [], ['proxy'])
	gen.bind_method(shared_sao_post_process, 'SetStrength', 'void', ['float strength'], ['proxy'])
	gen.end_class(shared_sao_post_process)

	# hg::BloomPostProcess
	bloom_post_process = gen.begin_class('hg::BloomPostProcess', bound_name='BloomPostProcess_nobind', noncopyable=True, nobind=True)
	gen.end_class(bloom_post_process)
	
	shared_bloom_post_process = gen.begin_class('std::shared_ptr<hg::BloomPostProcess>', bound_name='BloomPostProcess', features={'proxy': lib.stl.SharedPtrProxyFeature(bloom_post_process)})
	gen.add_base(shared_bloom_post_process, shared_post_process_component)
	gen.bind_constructor(shared_bloom_post_process, [], ['proxy'])
	gen.bind_method(shared_bloom_post_process, 'GetThreshold', 'float', [], ['proxy'])
	gen.bind_method(shared_bloom_post_process, 'SetThreshold', 'void', ['float threshold'], ['proxy'])
	gen.bind_method(shared_bloom_post_process, 'GetSmoothness', 'float', [], ['proxy'])
	gen.bind_method(shared_bloom_post_process, 'SetSmoothness', 'void', ['float smoothness'], ['proxy'])
	gen.bind_method(shared_bloom_post_process, 'GetBlur', 'float', [], ['proxy'])
	gen.bind_method(shared_bloom_post_process, 'SetBlur', 'void', ['float blur'], ['proxy'])
	gen.bind_method(shared_bloom_post_process, 'GetRadius', 'float', [], ['proxy'])
	gen.bind_method(shared_bloom_post_process, 'SetRadius', 'void', ['float radius'], ['proxy'])
	gen.bind_method(shared_bloom_post_process, 'GetIntensity', 'float', [], ['proxy'])
	gen.bind_method(shared_bloom_post_process, 'SetIntensity', 'void', ['float intensity'], ['proxy'])
	gen.bind_method(shared_bloom_post_process, 'GetQuality', 'int', [], ['proxy'])
	gen.bind_method(shared_bloom_post_process, 'SetQuality', 'void', ['int quality'], ['proxy'])
	gen.end_class(shared_bloom_post_process)
	...
	gen.insert_binding_code("""
static std::shared_ptr<hg::RenderSystemAsync> _ScriptEngineEnv_GetRenderSystemAsync(hg::ScriptEngineEnv *env) { return env->render_system_async; }
static std::shared_ptr<hg::RendererAsync> _ScriptEngineEnv_GetRendererAsync(hg::ScriptEngineEnv *env) { return env->renderer_async; }
static std::shared_ptr<hg::MixerAsync> _ScriptEngineEnv_GetMixerAsync(hg::ScriptEngineEnv *env) { return env->mixer_async; }
static float _ScriptEngineEnv_GetClockDt(hg::ScriptEngineEnv *env) { return env->dt; }
""")
	...
	gen.bind_method(shared_script_env, 'GetRenderSystemAsync', 'std::shared_ptr<hg::RenderSystemAsync>', [], {'proxy': None, 'route': route_lambda('_ScriptEngineEnv_GetRenderSystemAsync')})
	gen.bind_method(shared_script_env, 'GetRendererAsync', 'std::shared_ptr<hg::RendererAsync>', [], {'proxy': None, 'route': route_lambda('_ScriptEngineEnv_GetRendererAsync')})
	gen.bind_method(shared_script_env, 'GetMixerAsync', 'std::shared_ptr<hg::MixerAsync>', [], {'proxy': None, 'route': route_lambda('_ScriptEngineEnv_GetMixerAsync')})
	gen.bind_method(shared_script_env, 'GetClockDt', 'float', [], {'proxy': None, 'route': route_lambda('_ScriptEngineEnv_GetClockDt')})
...
''' % self.from_c_func
	...
	#
	bind_navigation(gen, shared_scene_system, shared_component)
	...
	gen.bind_method(shared_node, 'AddComponent', 'void', ['std::shared_ptr<hg::Component> component', '?int index'], ['proxy'])
	...
	gen.bind_method(shared_node, 'GetComponent<hg::Transform>', 'std::shared_ptr<hg::Transform>', [], {'proxy': None}, bound_name='GetTransform')
	gen.bind_method(shared_node, 'GetComponent<hg::Camera>', 'std::shared_ptr<hg::Camera>', [], {'proxy': None}, bound_name='GetCamera')
	gen.bind_method(shared_node, 'GetComponent<hg::Object>', 'std::shared_ptr<hg::Object>', [], {'proxy': None}, bound_name='GetObject')
	gen.bind_method(shared_node, 'GetComponent<hg::Light>', 'std::shared_ptr<hg::Light>', [], {'proxy': None}, bound_name='GetLight')
	gen.bind_method(shared_node, 'GetComponent<hg::Instance>', 'std::shared_ptr<hg::Instance>', [], {'proxy': None}, bound_name='GetInstance')
	gen.bind_method(shared_node, 'GetComponent<hg::Target>', 'std::shared_ptr<hg::Target>', [], {'proxy': None}, bound_name='GetTarget')
	gen.bind_method(shared_node, 'GetComponent<hg::RigidBody>', 'std::shared_ptr<hg::RigidBody>', [], {'proxy': None}, bound_name='GetRigidBody')
	gen.bind_method(shared_node, 'GetComponent<hg::BoxCollision>', 'std::shared_ptr<hg::BoxCollision>', [], {'proxy': None}, bound_name='GetBoxCollision')
	gen.bind_method(shared_node, 'GetComponent<hg::SphereCollision>', 'std::shared_ptr<hg::SphereCollision>', [], {'proxy': None}, bound_name='GetSphereCollision')
	gen.bind_method(shared_node, 'GetComponent<hg::MeshCollision>', 'std::shared_ptr<hg::MeshCollision>', [], {'proxy': None}, bound_name='GetMeshCollision')
	gen.bind_method(shared_node, 'GetComponent<hg::CapsuleCollision>', 'std::shared_ptr<hg::CapsuleCollision>', [], {'proxy': None}, bound_name='GetCapsuleCollision')
	gen.bind_method(shared_node, 'GetComponent<hg::ConvexCollision>', 'std::shared_ptr<hg::ConvexCollision>', [], {'proxy': None}, bound_name='GetConvexCollision')
	gen.bind_method(shared_node, 'GetComponent<hg::SphericalJoint>', 'std::shared_ptr<hg::SphericalJoint>', [], {'proxy': None}, bound_name='GetSphericalJoint')
	gen.bind_method(shared_node, 'GetComponent<hg::D6Joint>', 'std::shared_ptr<hg::D6Joint>', [], {'proxy': None}, bound_name='GetD6Joint')
	gen.bind_method(shared_node, 'GetComponent<hg::RenderScript>', 'std::shared_ptr<hg::RenderScript>', [], {'proxy': None}, bound_name='GetRenderScript')
	gen.bind_method(shared_node, 'GetComponent<hg::LogicScript>', 'std::shared_ptr<hg::LogicScript>', [], {'proxy': None}, bound_name='GetLogicScript')
	gen.bind_method(shared_node, 'GetComponent<hg::Terrain>', 'std::shared_ptr<hg::Terrain>', [], {'proxy': None}, bound_name='GetTerrain')

	gen.bind_method(shared_node, 'HasAspect', 'bool', ['const std::string &aspect'], ['proxy'])
	gen.bind_method(shared_node, 'IsReady', 'bool', [], ['proxy'])

	gen.bind_method(shared_node, 'GetInstantiatedByNode', 'const std::shared_ptr<hg::Node> &', [], ['proxy'])
	gen.bind_method(shared_node, 'GetInstantiatedByScene', 'const std::shared_ptr<hg::Scene> &', [], ['proxy'])

	gen.bind_method_overloads(shared_node, 'SetInstantiatedBy', [
		('void', ['std::shared_ptr<hg::Node> node'], ['proxy']),
		('void', ['std::shared_ptr<hg::Scene> scene'], ['proxy'])
	])

	gen.bind_method(shared_node, 'IsInstantiated', 'bool', [], ['proxy'])

	decl_get_set_method(gen, shared_node, 'bool', 'Enabled', 'enable', features=['proxy'])
	decl_get_set_method(gen, shared_node, 'bool', 'IsStatic', 'is_static', features=['proxy'])
	decl_get_set_method(gen, shared_node, 'bool', 'DoNotSerialize', 'do_not_serialize', features=['proxy'])
	decl_get_set_method(gen, shared_node, 'bool', 'DoNotInstantiate', 'do_not_instantiate', features=['proxy'])
	decl_get_set_method(gen, shared_node, 'bool', 'UseForNavigation', 'use_for_navigation', features=['proxy'])

	gen.bind_method_overloads(shared_node, 'GetNode', [
		('std::shared_ptr<hg::Node>', ['uint32_t uid'], {'proxy': None}),
		('std::shared_ptr<hg::Node>', ['const std::string &name', '?const std::shared_ptr<hg::Node> &node'], {'proxy': None})
	])
...
\n''', 'PhysicTrace extension')
	...
	decl_get_set_method(gen, shared_physic_system, 'float', 'Timestep', 'timestep', ['proxy'])
	decl_get_set_method(gen, shared_physic_system, 'bool', 'ForceRigidBodyToSleepOnCreation', 'force_sleep_body', ['proxy'])
	decl_get_set_method(gen, shared_physic_system, 'uint32_t', 'ForceRigidBodyAxisLockOnCreation', 'force_axis_lock', ['proxy'])

	decl_get_set_method(gen, shared_physic_system, 'hg::Vector3', 'Gravity', 'G', ['proxy'])
	...
	decl_comp_get_set_method(gen, shared_physic_system, 'hg::RigidBody', 'rigid_body', 'bool', 'RigidBodyIsSleeping', 'sleeping', ['proxy'])

	decl_comp_get_set_method(gen, shared_physic_system, 'hg::RigidBody', 'rigid_body', 'hg::Vector3', 'RigidBodyAngularVelocity', 'W', ['proxy'])
	decl_comp_get_set_method(gen, shared_physic_system, 'hg::RigidBody', 'rigid_body', 'hg::Vector3', 'RigidBodyLinearVelocity', 'V', ['proxy'])

	decl_comp_get_set_method(gen, shared_physic_system, 'hg::RigidBody', 'rigid_body', 'float', 'RigidBodyLinearDamping', 'k', ['proxy'])
	decl_comp_get_set_method(gen, shared_physic_system, 'hg::RigidBody', 'rigid_body', 'float', 'RigidBodyAngularDamping', 'k', ['proxy'])

	decl_comp_get_set_method(gen, shared_physic_system, 'hg::RigidBody', 'rigid_body', 'float', 'RigidBodyStaticFriction', 'static_friction', ['proxy'])
	decl_comp_get_set_method(gen, shared_physic_system, 'hg::RigidBody', 'rigid_body', 'float', 'RigidBodyDynamicFriction', 'dynamic_friction', ['proxy'])

	decl_comp_get_set_method(gen, shared_physic_system, 'hg::RigidBody', 'rigid_body', 'float', 'RigidBodyRestitution', 'restitution', ['proxy'])

	#decl_comp_get_set_method(gen, shared_physic_system, 'hg::RigidBody', 'rigid_body', 'hg::RigidBodyType', 'RigidBodyType', 'type', ['proxy'])

	decl_comp_get_set_method(gen, shared_physic_system, 'hg::RigidBody', 'rigid_body', 'uint8_t', 'RigidBodyAxisLock', 'axis_lock', ['proxy'])
	decl_comp_get_set_method(gen, shared_physic_system, 'hg::RigidBody', 'rigid_body', 'uint8_t', 'RigidBodyCollisionLayer', 'layer', ['proxy'])
...
static std::shared_ptr<hg::PhysicSystem> CreatePhysicSystem() { return hg::g_physic_system_factory.get().Instantiate(); }
	...
	# hg::SceneBoundAnim
	gen.add_include('engine/scene_animation.h')

	scene_bound_anim = gen.begin_class('hg::SceneBoundAnim', bound_name='SceneBoundAnim_nobind', noncopyable=True, nobind=True)
	gen.end_class(scene_bound_anim)

	shared_scene_bound_anim = gen.begin_class('std::shared_ptr<hg::SceneBoundAnim>', bound_name='SceneBoundAnim', features={'proxy': lib.stl.SharedPtrProxyFeature(scene_bound_anim)})
	gen.end_class(shared_scene_bound_anim)

	# hg::AnimTrack
	anim_track = gen.begin_class('hg::AnimTrack', bound_name='AnimTrack_nobind', noncopyable=True, nobind=True)
	gen.end_class(anim_track)

	gen.insert_binding_code('''\
static std::string _AnimTrack_GetTarget(const hg::AnimTrack *track) { return track->target; }
static void _AnimTrack_SetTarget(hg::AnimTrack *track, const std::string &target) { track->target = target; }
''')

	def AnimTrackT_rval_transform(gen, conv, expr, var_out, ownership):
		src = 'std::string type = (*(%s))->GetType();\n' % expr
		src += 'hg::Type *storage_type = (*(%s))->GetStorageType();\n' % expr
		src += '''
static hg::Type
	*float_type = hg::g_type_registry.get().GetType<float>(),
	*vector3_type = hg::g_type_registry.get().GetType<hg::Vector3>();
\n'''
		src += 'if (type == "AnimTrackRotation") {\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::AnimTrackRotation>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::AnimTrackRotation> *>(%s)' % expr, ownership)
		src += '} else {\n'
		src += '	if (storage_type == float_type) {\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::AnimTrackT<float>>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::AnimTrackT<float>> *>(%s)' % expr, ownership)
		src += '	} else if (storage_type == vector3_type) {\n'
		src += gen.rval_from_c_ptr(gen.get_conv('std::shared_ptr<hg::AnimTrackT<hg::Vector3>>'), var_out, 'reinterpret_cast<std::shared_ptr<hg::AnimTrackT<hg::Vector3>> *>(%s)' % expr, ownership)
		src += '	} else {\n'
		src += gen.rval_from_c_ptr(conv, var_out, expr, ownership)
		src += '	}\n'
		src += '}\n'
	...
	shared_anim_track = gen.begin_class('std::shared_ptr<hg::AnimTrack>', bound_name='AnimTrack', features={'proxy': lib.stl.SharedPtrProxyFeature(anim_track)})
	shared_anim_track.add_feature('rval_transform', AnimTrackT_rval_transform)

	gen.bind_method(shared_anim_track, 'GetTarget', 'std::string', [], {'proxy': None, 'route': route_lambda('_AnimTrack_GetTarget')})
	gen.bind_method(shared_anim_track, 'SetTarget', 'void', ['const std::string &target'], {'proxy': None, 'route': route_lambda('_AnimTrack_SetTarget')})

	gen.bind_method(shared_anim_track, 'GetDuration', 'hg::time_ns', [], ['proxy'])
	gen.bind_method(shared_anim_track, 'GetRange', 'bool', ['hg::time_ns &t_start', 'hg::time_ns &t_end'], {'proxy': None, 'arg_out': ['t_start', 't_end']})

	gen.end_class(shared_anim_track)

	def bind_AnimKeyT(T, T_type_name):
		anim_keyframeT = gen.begin_class('hg::AnimKey<%s>' % T, bound_name='AnimKey' + T_type_name)
		gen.bind_members(anim_keyframeT, ['hg::time_ns t', '%s v' % T])
		gen.end_class(anim_keyframeT)

	def bind_AnimTrackT(T, T_type_name, bound_name=None, concrete_type=None):
		if bound_name is None:
			bound_name = "AnimTrack" + T_type_name
		if concrete_type is None:
			concrete_type = 'hg::AnimTrackT<%s>' % T

		anim_trackT = gen.begin_class(concrete_type, bound_name=bound_name + '_nobind', noncopyable=True, nobind=True)
		gen.end_class(anim_trackT)

		gen.insert_binding_code('static bool _%s_GetValue(hg::AnimTrack *track, hg::time_ns t, %s &val_out) { return track->GetValue(t, &val_out); }' % (bound_name, T))
		gen.insert_binding_code('static size_t _%s_GetKeyCount(const %s *track) { return track->GetKeys().size(); }' % (bound_name, concrete_type))
		gen.insert_binding_code('static hg::AnimKey<%s> _%s_GetKey(const %s *track, size_t idx) { return track->GetKeys()[idx]; }' % (T, bound_name, concrete_type))

		shared_anim_trackT = gen.begin_class('std::shared_ptr<%s>' % concrete_type, bound_name=bound_name, features={'proxy': lib.stl.SharedPtrProxyFeature(anim_trackT)})
		gen.add_base(shared_anim_trackT, shared_anim_track)
		gen.bind_constructor(shared_anim_trackT, ['const std::string &target'], ['proxy'])
		gen.bind_method(shared_anim_trackT, 'GetValue', 'bool', ['hg::time_ns t', '%s &value' % T], {'proxy': None, 'arg_out': ['value'], 'route': route_lambda('_%s_GetValue' % bound_name)})
		gen.bind_method(shared_anim_trackT, 'GetKeyCount', 'size_t', [], {'proxy': None, 'route': route_lambda('_%s_GetKeyCount' % bound_name)})
		gen.bind_method(shared_anim_trackT, 'GetKey', 'hg::AnimKey<%s>' % T, ['size_t idx'], {'proxy': None, 'route': route_lambda('_%s_GetKey' % bound_name)})
		gen.bind_method(shared_anim_trackT, 'SetKey', 'void', ['hg::time_ns t', 'const %s &v' % T], ['proxy'])
		gen.bind_method_overloads(shared_anim_trackT, 'RemoveKey', [
			('bool', ['hg::time_ns t'], ['proxy']),
			('bool', ['size_t idx'], ['proxy'])
		])
		gen.bind_method(shared_anim_trackT, 'Clear', 'void', [], ['proxy'])
		gen.end_class(shared_anim_trackT)

	bind_AnimKeyT('float', 'Float')
	bind_AnimKeyT('hg::Vector3', 'Vector3')

	bind_AnimTrackT('float', 'Float')
	bind_AnimTrackT('hg::Vector3', 'Vector3')
	bind_AnimTrackT('hg::Vector3', 'Vector3', 'AnimTrackRotation', 'hg::AnimTrackRotation')

	# hg::AnimLoop
	gen.add_include('engine/animation.h')

	gen.bind_named_enum('hg::AnimLoop', ['AnimOnce', 'AnimRepeat', 'AnimPingPong'])

	gen.bind_function('hg::TimeRepeat', 'hg::time_ns', ['hg::time_ns t', 'hg::time_ns start', 'hg::time_ns end'])
	gen.bind_function('hg::TimePingPong', 'hg::time_ns', ['hg::time_ns t', 'hg::time_ns start', 'hg::time_ns end'])

	# hg::Anim
	anim = gen.begin_class('hg::Anim', bound_name='Anim_nobind', noncopyable=True, nobind=True)
	gen.end_class(anim)

	gen.insert_binding_code('''
static const std::string &_Anim_GetTarget(hg::Anim *anim) { return anim->target; }
static const void _Anim_SetTarget(hg::Anim *anim, const std::string &target) { anim->target = target; }
static size_t _Anim_GetTrackCount(hg::Anim *anim) { return anim->tracks.size(); }

static void _Anim_AddTrack(hg::Anim *anim, std::shared_ptr<hg::AnimTrack> track) { anim->tracks.push_back(std::move(track)); }

static std::shared_ptr<hg::AnimTrack> _Anim_GetTrackByIndex(hg::Anim *anim, size_t i) { return anim->tracks[i]; }
static std::shared_ptr<hg::AnimTrack> _Anim_GetTrackByTarget(hg::Anim *anim, const std::string &target) {
...
static bool _Anim_RemoveTrack(hg::Anim *anim, size_t idx) {
	if (idx >= anim->tracks.size())
		return false;
	anim->tracks.erase(std::begin(anim->tracks) + idx);
	return true;
}
static void _Anim_RemoveTrack(hg::Anim *anim, const std::string &target) {
	anim->tracks.erase(std::remove_if(std::begin(anim->tracks), std::end(anim->tracks), [&](const std::shared_ptr<hg::AnimTrack> &track) { return track->target == target; }), std::end(anim->tracks));
}
static void _Anim_RemoveTrack(hg::Anim *anim, const std::shared_ptr<hg::AnimTrack> &track) {
	anim->tracks.erase(std::remove_if(std::begin(anim->tracks), std::end(anim->tracks), [&](const std::shared_ptr<hg::AnimTrack> &track_) { return track_ == track; }), std::end(anim->tracks));
}
	''')

	shared_anim = gen.begin_class('std::shared_ptr<hg::Anim>', bound_name='Anim', features={'proxy': lib.stl.SharedPtrProxyFeature(anim)})
	gen.bind_constructor(shared_anim, ['const std::string &target'], ['proxy'])
	gen.bind_method(shared_anim, 'GetTarget', 'const std::string &', [], {'proxy': None, 'route': route_lambda('_Anim_GetTarget')})
	gen.bind_method(shared_anim, 'SetTarget', 'void', ['const std::string &target'], {'proxy': None, 'route': route_lambda('_Anim_SetTarget')})
	gen.bind_method(shared_anim, 'GetTrackCount', 'size_t', [], {'proxy': None, 'route': route_lambda('_Anim_GetTrackCount')})
	gen.bind_method(shared_anim, 'AddTrack', 'void', ['std::shared_ptr<hg::AnimTrack> track'], {'proxy': None, 'route': route_lambda('_Anim_AddTrack')})
	gen.bind_method_overloads(shared_anim, 'GetTrack', [
		('std::shared_ptr<hg::AnimTrack>', ['size_t idx'], {'proxy': None, 'route': route_lambda('_Anim_GetTrackByIndex'), 'check_rval': check_bool_rval_lambda(gen, 'Invalid animation track index')}),
		('std::shared_ptr<hg::AnimTrack>', ['const std::string &target'], {'proxy': None, 'route': route_lambda('_Anim_GetTrackByTarget')})
	])
	gen.bind_method_overloads(shared_anim, 'RemoveTrack', [
		('bool', ['size_t idx'], {'proxy': None, 'route': route_lambda('_Anim_RemoveTrack'), 'check_rval': check_bool_rval_lambda(gen, 'Invalid animation track index')}),
		('void', ['const std::string &target'], {'proxy': None, 'route': route_lambda('_Anim_RemoveTrack')}),
		('void', ['const std::shared_ptr<hg::AnimTrack> &track'], {'proxy': None, 'route': route_lambda('_Anim_RemoveTrack')})
	])
	gen.bind_method(shared_anim, 'GetTracksRange', 'bool', ['hg::time_ns &t_start', 'hg::time_ns &t_end'], {'proxy': None, 'arg_out': ['t_start', 't_end']})
	gen.end_class(shared_anim)

	# hg::AnimTake
	anim_take = gen.begin_class('hg::AnimTake', bound_name='AnimTake_nobind', noncopyable=True, nobind=True)
	gen.end_class(anim_take)

	gen.insert_binding_code('''
static const std::string &_AnimTake_GetName(hg::AnimTake *take) { return take->name; }
static const std::string &_AnimTake_SetName(hg::AnimTake *take, const std::string &name) { return take->name = name; }

static void _AnimTake_GetRange(hg::AnimTake *take, hg::time_ns &start, hg::time_ns &end) {
	start = take->start;
	end = take->end;
}
static void _AnimTake_SetRange(hg::AnimTake *take, hg::time_ns start, hg::time_ns end) {
	take->start = start;
	take->end = end;
}

static size_t _AnimTake_GetAnimCount(hg::AnimTake *take) { return take->anims.size(); }

static void _AnimTake_AddAnim(hg::AnimTake *take, std::shared_ptr<hg::Anim> anim) { take->anims.push_back(std::move(anim)); }

static std::shared_ptr<hg::Anim> _AnimTake_GetAnimByIndex(hg::AnimTake *take, size_t idx) { return idx < take->anims.size() ? take->anims[idx] : nullptr; }
static std::shared_ptr<hg::Anim> _AnimTake_GetAnimByTarget(hg::AnimTake *take, const std::string &target) {
	for (auto &anim : take->anims)
		if (anim->target == target)
			return anim;
	return nullptr;
}

static bool _AnimTake_RemoveAnim(hg::AnimTake *take, size_t idx) {
	if (idx >= take->anims.size())
		return false;
	take->anims.erase(std::begin(take->anims) + idx);
	return true;
}
static void _AnimTake_RemoveAnim(hg::AnimTake *take, const std::string &target) {
	take->anims.erase(std::remove_if(std::begin(take->anims), std::end(take->anims), [&](const std::shared_ptr<hg::Anim> &anim) { return anim->target == target; }), std::end(take->anims));
}
static void _AnimTake_RemoveAnim(hg::AnimTake *take, const std::shared_ptr<hg::Anim> &anim) {
	take->anims.erase(std::remove_if(std::begin(take->anims), std::end(take->anims), [&](const std::shared_ptr<hg::Anim> &anim_) { return anim_ == anim; }), std::end(take->anims));
}
	''')

	shared_anim_take = gen.begin_class('std::shared_ptr<hg::AnimTake>', bound_name='AnimTake', features={'proxy': lib.stl.SharedPtrProxyFeature(anim_take)})
	gen.bind_constructor(shared_anim_take, ['const std::string &name'], ['proxy'])
	gen.bind_method(shared_anim_take, 'GetName', 'const std::string &', [], {'proxy': None, 'route': route_lambda('_AnimTake_GetName')})
	gen.bind_method(shared_anim_take, 'SetName', 'void', ['const std::string &name'], {'proxy': None, 'route': route_lambda('_AnimTake_SetName')})
	gen.bind_method(shared_anim_take, 'GetRange', 'void', ['hg::time_ns &start', 'hg::time_ns &end'], {'proxy': None, 'arg_out': ['start', 'end'], 'route': route_lambda('_AnimTake_GetRange')})
	gen.bind_method(shared_anim_take, 'SetRange', 'void', ['hg::time_ns start', 'hg::time_ns end'], {'proxy': None, 'route': route_lambda('_AnimTake_SetRange')})
	gen.bind_method(shared_anim_take, 'GetAnimCount', 'size_t', [], {'proxy': None, 'route': route_lambda('_AnimTake_GetAnimCount')})
	gen.bind_method(shared_anim_take, 'AddAnim', 'void', ['std::shared_ptr<hg::Anim> anim'], {'proxy': None, 'route': route_lambda('_AnimTake_AddAnim')})
	gen.bind_method_overloads(shared_anim_take, 'GetAnim', [
		('std::shared_ptr<hg::Anim>', ['size_t idx'], {'proxy': None, 'route': route_lambda('_AnimTake_GetAnimByIndex'), 'check_rval': check_bool_rval_lambda(gen, 'Invalid animation index')}),
		('std::shared_ptr<hg::Anim>', ['const std::string &target'], {'proxy': None, 'route': route_lambda('_AnimTake_GetAnimByTarget')})
	])
	gen.bind_method_overloads(shared_anim_take, 'RemoveAnim', [
		('bool', ['size_t idx'], {'proxy': None, 'route': route_lambda('_AnimTake_RemoveAnim'), 'check_rval': check_bool_rval_lambda(gen, 'Invalid animation index')}),
		('void', ['const std::string &target'], {'proxy': None, 'route': route_lambda('_AnimTake_RemoveAnim')}),
		('void', ['const std::shared_ptr<hg::Anim> &anim'], {'proxy': None, 'route': route_lambda('_AnimTake_RemoveAnim')})
	])
	gen.bind_method(shared_anim_take, 'GetAnimsRange', 'bool', ['hg::time_ns &t_start', 'hg::time_ns &t_end'], {'proxy': None, 'arg_out': ['t_start', 't_end']})
	gen.end_class(shared_anim_take)

	bind_std_vector(gen, shared_anim_take)
	...
	gen.bind_method(shared_group, 'GetAnimTakes', 'const std::vector<std::shared_ptr<hg::AnimTake>> &', [], ['proxy'])
	...
	gen.bind_method_overloads(shared_group, 'GetNode', [
		('std::shared_ptr<hg::Node>', ['uint32_t uid'], {'proxy': None}),
		('std::shared_ptr<hg::Node>', ['const std::string &name', '?const std::shared_ptr<hg::Node> &parent'], {'proxy': None})
	])

	gen.bind_method(shared_group, 'AddAnimTake', 'void', ['std::shared_ptr<hg::AnimTake> take'], ['proxy'])
	gen.bind_method(shared_group, 'RemoveAnimTake', 'void', ['const std::shared_ptr<hg::AnimTake> &take'], ['proxy'])

	gen.insert_binding_code('''
static std::shared_ptr<hg::AnimTake> _Group_GetAnimTakeByIndex(hg::Group *group, size_t i) {
	return i < group->GetAnimTakes().size() ? group->GetAnimTakes()[i] : nullptr;
}

static size_t _Group_GetAnimTakeCount(hg::Group *group) { return group->GetAnimTakes().size(); }
''')

	gen.bind_method_overloads(shared_group, 'GetAnimTake', [
		('std::shared_ptr<hg::AnimTake>', ['const std::string &name'], {'proxy': None}),
		('std::shared_ptr<hg::AnimTake>', ['size_t idx'], {'proxy': None, 'check_rval': check_bool_rval_lambda(gen, 'Invalid AnimTake index'), 'route': route_lambda('_Group_GetAnimTakeByIndex')})
	])
	gen.bind_method(shared_group, 'GetAnimTakeCount', 'size_t', [], {'proxy': None, 'route': route_lambda('_Group_GetAnimTakeCount')})
	...
	gen.bind_method(shared_scene, 'GetCurrentCamera', 'const std::shared_ptr<hg::Node> &', [], {'proxy': None})
	...
	gen.bind_method(shared_scene, 'RemoveGroupAndMembers', 'void', ['const std::shared_ptr<hg::Group> &group'], ['proxy'])
	...
	gen.bind_method(shared_scene, 'GetSystem<hg::RenderableSystem>', 'std::shared_ptr<hg::RenderableSystem>', [], {'proxy': None}, bound_name='GetRenderableSystem')
	gen.bind_method(shared_scene, 'GetSystem<hg::PhysicSystem>', 'std::shared_ptr<hg::PhysicSystem>', [], {'proxy': None}, bound_name='GetPhysicSystem')
	...
	gen.bind_method(shared_scene, 'AddComponent', 'void', ['std::shared_ptr<hg::Component> component', '?int index'], ['proxy'])
	...
	gen.bind_method(shared_scene, 'GetComponent<hg::Environment>', 'std::shared_ptr<hg::Environment>', [], {'proxy': None}, bound_name='GetEnvironment')	
	gen.bind_method(shared_scene, 'GetComponent<hg::SimpleGraphicSceneOverlay>', 'std::shared_ptr<hg::SimpleGraphicSceneOverlay>', [], {'proxy': None}, bound_name='GetSimpleGraphicSceneOverlay')
	...
	std::shared_ptr<SceneBoundAnim> StartBoundAnims(const std::vector<std::shared_ptr<BoundAnim>> &bound_anims, AnimLoop loop = AnimOnce, time_ns t_start = 0, float t_scale = 1.f);
	"""

	gen.insert_binding_code('''
static size_t _Scene_GetAnimTakeCount(hg::Scene *scene) { return scene->anim_takes.size(); }

static void _Scene_AddAnimTake(hg::Scene *scene, std::shared_ptr<hg::AnimTake> take) { scene->anim_takes.push_back(std::move(take)); }

static std::shared_ptr<hg::AnimTake> _Scene_GetAnimTakeByName(hg::Scene *scene, const std::string &name) {
	for (auto &anim_take : scene->anim_takes)
		if (anim_take->name == name)
			return anim_take;
	return {};
}
static std::shared_ptr<hg::AnimTake> _Scene_GetAnimTakeByIndex(hg::Scene *scene, size_t i) {
	return i < scene->anim_takes.size() ? scene->anim_takes[i] : nullptr;
}

static bool _Scene_RemoveAnimTake(hg::Scene *scene, size_t idx) {
	if (idx >= scene->anim_takes.size())
		return false;
	scene->anim_takes.erase(std::begin(scene->anim_takes) + idx);
	return true;
}
static void _Scene_RemoveAnimTake(hg::Scene *scene, const std::string &name) {
	scene->anim_takes.erase(std::remove_if(std::begin(scene->anim_takes), std::end(scene->anim_takes), [&](const std::shared_ptr<hg::AnimTake> &take) { return take->name == name; }), std::end(scene->anim_takes));
}
static void _Scene_RemoveAnimTake(hg::Scene *scene, const std::shared_ptr<hg::AnimTake> &take) {
	scene->anim_takes.erase(std::remove_if(std::begin(scene->anim_takes), std::end(scene->anim_takes), [&](const std::shared_ptr<hg::AnimTake> &take_) { return take_ == take; }), std::end(scene->anim_takes));
}
''')

	gen.bind_method(shared_scene, 'GetAnimTakeCount', 'size_t', [], {'proxy': None, 'route': route_lambda('_Scene_GetAnimTakeCount')})
	gen.bind_method(shared_scene, 'AddAnimTake', 'void', ['std::shared_ptr<hg::AnimTake> take'], {'proxy': None, 'route': route_lambda('_Scene_AddAnimTake')})
	gen.bind_method_overloads(shared_scene, 'GetAnimTake', [
		('std::shared_ptr<hg::AnimTake>', ['const std::string &name'], {'proxy': None, 'route': route_lambda('_Scene_GetAnimTakeByName')}),
		('std::shared_ptr<hg::AnimTake>', ['size_t idx'], {'proxy': None, 'check_rval': check_bool_rval_lambda(gen, 'Invalid AnimTake index'), 'route': route_lambda('_Scene_GetAnimTakeByIndex')})
	])
	gen.bind_method_overloads(shared_scene, 'RemoveAnimTake', [
		('bool', ['size_t idx'], {'proxy': None, 'route': route_lambda('_Scene_RemoveAnimTake'), 'check_rval': check_bool_rval_lambda(gen, 'Invalid animation take index')}),
		('void', ['const std::string &name'], {'proxy': None, 'route': route_lambda('_Scene_RemoveAnimTake')}),
		('void', ['const std::shared_ptr<hg::AnimTake> &take'], {'proxy': None, 'route': route_lambda('_Scene_RemoveAnimTake')})
	])

	gen.bind_method_overloads(shared_scene, 'StartAnimTake', [
		('std::shared_ptr<hg::SceneBoundAnim>', ['const std::shared_ptr<hg::AnimTake> &take', '?hg::AnimLoop loop', '?hg::time_ns t_start', '?float t_scale'], {'proxy': None, 'check_rval': check_bool_rval_lambda(gen, 'StartAnimTake failed')}),
		('std::shared_ptr<hg::SceneBoundAnim>', ['const std::string &name', '?hg::AnimLoop loop', '?hg::time_ns t_start', '?float t_scale'], {'proxy': None, 'check_rval': check_bool_rval_lambda(gen, 'StartAnimTake failed')}),
		('std::shared_ptr<hg::SceneBoundAnim>', ['const std::shared_ptr<hg::AnimTake> &take', 'const std::vector<std::shared_ptr<hg::Node>> &nodes', '?hg::AnimLoop loop', '?hg::time_ns t_start', '?float t_scale'], {'proxy': None, 'check_rval': check_bool_rval_lambda(gen, 'StartAnimTake failed')}),
		('std::shared_ptr<hg::SceneBoundAnim>', ['const std::string &name', 'const std::vector<std::shared_ptr<hg::Node>> &nodes', '?hg::AnimLoop loop', '?hg::time_ns t_start', '?float t_scale'], {'proxy': None, 'check_rval': check_bool_rval_lambda(gen, 'StartAnimTake failed')})
	])
	gen.bind_method_overloads(shared_scene, 'StartAnimTakeRange', [
		('std::shared_ptr<hg::SceneBoundAnim>', ['const std::shared_ptr<hg::AnimTake> &take', 'hg::time_ns t_range_start', 'hg::time_ns t_range_end', '?hg::AnimLoop loop', '?hg::time_ns t_start', '?float t_scale'], {'proxy': None, 'check_rval': check_bool_rval_lambda(gen, 'StartAnimTake failed')}),
		('std::shared_ptr<hg::SceneBoundAnim>', ['const std::string &name', 'hg::time_ns t_range_start', 'hg::time_ns t_range_end', '?hg::AnimLoop loop', '?hg::time_ns t_start', '?float t_scale'], {'proxy': None, 'check_rval': check_bool_rval_lambda(gen, 'StartAnimTake failed')}),
		('std::shared_ptr<hg::SceneBoundAnim>', ['const std::shared_ptr<hg::AnimTake> &take', 'const std::vector<std::shared_ptr<hg::Node>> &nodes', 'hg::time_ns t_range_start', 'hg::time_ns t_range_end', '?hg::AnimLoop loop', '?hg::time_ns t_start', '?float t_scale'], {'proxy': None, 'check_rval': check_bool_rval_lambda(gen, 'StartAnimTake failed')}),
		('std::shared_ptr<hg::SceneBoundAnim>', ['const std::string &name', 'const std::vector<std::shared_ptr<hg::Node>> &nodes', 'hg::time_ns t_range_start', 'hg::time_ns t_range_end', '?hg::AnimLoop loop', '?hg::time_ns t_start', '?float t_scale'], {'proxy': None, 'check_rval': check_bool_rval_lambda(gen, 'StartAnimTake failed')})
	])

	gen.bind_method(shared_scene, 'StartGroupAnimTake', 'std::shared_ptr<hg::SceneBoundAnim>', ['const std::shared_ptr<hg::Group> &group', 'const std::string &name', '?hg::AnimLoop loop', '?hg::time_ns t_start', '?float t_scale'], {'proxy': None, 'check_rval': check_bool_rval_lambda(gen, 'StartGroupAnimTake failed')})
	gen.bind_method(shared_scene, 'StartGroupAnimTakeRange', 'std::shared_ptr<hg::SceneBoundAnim>', ['const std::shared_ptr<hg::Group> &group', 'const std::string &name', 'hg::time_ns t_range_start', 'hg::time_ns t_range_end', '?hg::AnimLoop loop', '?hg::time_ns t_start', '?float t_scale'], {'proxy': None, 'check_rval': check_bool_rval_lambda(gen, 'StartGroupAnimTakeRange failed')})

	gen.bind_method(shared_scene, 'StopAnim', 'void', ['const std::shared_ptr<hg::SceneBoundAnim> &anim'], ['proxy'])

	gen.bind_method(shared_scene, 'GetAnimT', 'hg::time_ns', ['const std::shared_ptr<hg::SceneBoundAnim> &anim'], ['proxy'])
	gen.bind_method(shared_scene, 'SetAnimT', 'void', ['const std::shared_ptr<hg::SceneBoundAnim> &anim', 'hg::time_ns t'], ['proxy'])

	gen.bind_method(shared_scene, 'GetAnimTScale', 'float', ['const std::shared_ptr<hg::SceneBoundAnim> &anim'], ['proxy'])
	gen.bind_method(shared_scene, 'SetAnimTScale', 'void', ['const std::shared_ptr<hg::SceneBoundAnim> &anim', 'float t_scale'], ['proxy'])

	gen.bind_method(shared_scene, 'IsAnimLooped', 'bool', ['const std::shared_ptr<hg::SceneBoundAnim> &anim'], ['proxy'])
...
''')
	...
	lib.stl.bind_future_T(gen, 'std::shared_ptr<hg::RenderTarget>', 'FutureRenderTarget')
	...
	gen.bind_method(shared_renderer, 'ClearCache', 'void', [], ['proxy'])
...
\n''')
	...
	gen.bind_method(shared_renderer, 'GetTexture', 'std::shared_ptr<hg::Texture>', ['const std::string &path'], {'proxy': None})
	...
	gen.bind_method(shared_renderer, 'GetShader', 'std::shared_ptr<hg::GpuShader>', ['?const std::string &name'], {'proxy': None})
	...
	def bind_set_shader_var(class_binding, alias, type, count, is_async=False):
			renderer_type = "Renderer%s" % ("Async" if is_async else "")
			method_name = "SetShader%s%s" % (alias, str(count) if count>1 else "")
			alias_name = "_%s_%s" % (renderer_type, method_name)
			async_get = '.get()' if is_async else ''
			data = list(map(lambda x: "v%d"%x, range(count)))
			method_args = list(map(lambda x: "%s %s"%(type,x), data))
			args = (alias_name, renderer_type, ', '.join(method_args), type, ','.join(data), method_name)
			gen.insert_binding_code('''
	static void %s(hg::%s *renderer, const hg::GpuShaderVariable &var, %s) {
		%s v[] = {%s};
		renderer->%s(var, v); 
	}''' % tuple(args))
			args = (alias_name, renderer_type, ', '.join(method_args), type, ','.join(data), method_name, async_get)
			gen.insert_binding_code('''
	static void %s_name(hg::%s *renderer, const std::string &name, %s) {
		%s v[] = {%s};
		renderer->%s(renderer->GetShaderVariable(name)%s, v); 
	}
	''' % tuple(args))
			args = (alias_name, renderer_type, type, method_name, count)
			gen.insert_binding_code('''
	static void %s_vector(hg::%s *renderer, const hg::GpuShaderVariable &var, const std::vector<%s> &v) {
		renderer->%s(var, v.data(), v.size()/%d); 
	}	''' % tuple(args))
			args = (alias_name, renderer_type, type, method_name, async_get, count)
			gen.insert_binding_code('''	
	static void %s_vector(hg::%s *renderer, const std::string &name, const std::vector<%s> &v) {
		renderer->%s(renderer->GetShaderVariable(name)%s, v.data(), v.size()/%d); 
	}
	''' % (tuple(args)))
			gen.bind_method_overloads(class_binding, method_name, expand_std_vector_proto(gen, [
		('void', ['const hg::GpuShaderVariable &var'] + method_args, {'proxy': None, 'route': route_lambda(alias_name)}),
		('void', ['const std::string &name'] + method_args, {'proxy': None, 'route': route_lambda('%s_name' % alias_name)}),
		('void', ['const hg::GpuShaderVariable &var', 'const std::vector<%s>& v' % type], {'proxy': None, 'route': route_lambda('%s_vector' % alias_name)}),
		('void', ['const std::string &name', 'const std::vector<%s>& v' % type], {'proxy': None, 'route': route_lambda('%s_vector' % alias_name)})
	]))

	for n in range(1,5) : bind_set_shader_var(shared_renderer, 'Int', 'int', n) 
	for n in range(1,5) : bind_set_shader_var(shared_renderer, 'Unsigned', 'uint32_t', n) 
	for n in range(1,5) : bind_set_shader_var(shared_renderer, 'Float', 'float', n) 
...
static std::shared_ptr<hg::Renderer> CreateRenderer() { return hg::g_renderer_factory.get().Instantiate(); }
	...
	gen.bind_method(shared_renderer_async, 'ClearCache', 'std::future<void>', [], ['proxy'])
	...
	gen.bind_method(shared_renderer_async, 'GetRenderTarget', 'std::future<std::shared_ptr<hg::RenderTarget>>', [], ['proxy'])
	...
	#std::future<void *> MapBuffer(sBuffer buf) {
	#void UnmapBuffer(sBuffer buf) { run_call<void>(std::bind(&Renderer::UnmapBuffer, shared_ref(renderer), shared_ref(buf)), RA_task_affinity); }
	gen.bind_method(shared_renderer_async, 'CreateBuffer', 'std::future<bool>', ['std::shared_ptr<hg::GpuBuffer> buf', 'const hg::BinaryData &data', 'hg::GpuBufferType type', '?hg::GpuBufferUsage usage'], ['proxy'])
	gen.bind_method(shared_renderer_async, 'FreeBuffer', 'void', ['std::shared_ptr<hg::GpuBuffer> buffer'], ['proxy'])
	gen.bind_method(shared_renderer_async, 'DrawBuffers', 'void', ['uint32_t index_count', 'const std::shared_ptr<hg::GpuBuffer> &idx', 'const std::shared_ptr<hg::GpuBuffer> &vtx', 'hg::VertexLayout &layout', '?hg::IndexType idx_type', '?hg::GpuPrimitiveType prim_type'], ['proxy'])
	gen.bind_method(shared_renderer_async, 'UpdateBuffer', 'std::future<bool>', ['std::shared_ptr<hg::GpuBuffer> buf', 'const hg::BinaryData &data', '?size_t start'], ['proxy'])
	...
	gen.bind_method_overloads(shared_renderer_async, 'LoadShader', [
		('std::shared_ptr<hg::GpuShader>', ['const std::string &name', '?bool use_cache'], ['proxy']),
		('std::shared_ptr<hg::GpuShader>', ['const std::string &name', 'const std::string &source', '?bool use_cache'], ['proxy'])
	])
	...
	for n in range(1,5) : bind_set_shader_var(shared_renderer_async, 'Int', 'int', n, True) 
	for n in range(1,5) : bind_set_shader_var(shared_renderer_async, 'Unsigned', 'uint32_t', n, True) 
	for n in range(1,5) : bind_set_shader_var(shared_renderer_async, 'Float', 'float', n, True) 

	gen.insert_binding_code('''\
static void _RendererAsync_SetShaderMatrix3(hg::RendererAsync *renderer, const hg::GpuShaderVariable &var, const hg::Matrix3 &m) { renderer->SetShaderMatrix3(var, &m); }
static void _RendererAsync_SetShaderMatrix4(hg::RendererAsync *renderer, const hg::GpuShaderVariable &var, const hg::Matrix4 &m) { renderer->SetShaderMatrix4(var, &m); }
static void _RendererAsync_SetShaderMatrix44(hg::RendererAsync *renderer, const hg::GpuShaderVariable &var, const hg::Matrix44 &m) { renderer->SetShaderMatrix44(var, &m); }

static void _RendererAsync_SetShaderTexture(hg::RendererAsync *renderer, const hg::GpuShaderVariable &var, const std::shared_ptr<hg::Texture> &t) { renderer->SetShaderTexture(var, t); }

static void _RendererAsync_SetShaderMatrix3_name(hg::RendererAsync *renderer, const std::string &name, const hg::Matrix3 &m) { renderer->SetShaderMatrix3(renderer->GetShaderVariable(name).get(), &m); }
static void _RendererAsync_SetShaderMatrix4_name(hg::RendererAsync *renderer, const std::string &name, const hg::Matrix4 &m) { renderer->SetShaderMatrix4(renderer->GetShaderVariable(name).get(), &m); }
static void _RendererAsync_SetShaderMatrix44_name(hg::RendererAsync *renderer, const std::string &name, const hg::Matrix44 &m) { renderer->SetShaderMatrix44(renderer->GetShaderVariable(name).get(), &m); }

static void _RendererAsync_SetShaderTexture_name(hg::RendererAsync *renderer, const std::string &name, const std::shared_ptr<hg::Texture> &t) { renderer->SetShaderTexture(renderer->GetShaderVariable(name).get(), t); }
''')

	gen.bind_method_overloads(shared_renderer_async, 'SetShaderMatrix3', [
		('void', ['const hg::GpuShaderVariable &var', 'const hg::Matrix3 &m'], {'proxy': None, 'route': route_lambda('_RendererAsync_SetShaderMatrix3')}),
		('void', ['const std::string &name', 'const hg::Matrix3 &m'], {'proxy': None, 'route': route_lambda('_RendererAsync_SetShaderMatrix3_name')})
	])
	gen.bind_method_overloads(shared_renderer_async, 'SetShaderMatrix4', [
		('void', ['const hg::GpuShaderVariable &var', 'const hg::Matrix4 &m'], {'proxy': None, 'route': route_lambda('_RendererAsync_SetShaderMatrix4')}),
		('void', ['const std::string &name', 'const hg::Matrix4 &m'], {'proxy': None, 'route': route_lambda('_RendererAsync_SetShaderMatrix4_name')})
	])
	gen.bind_method_overloads(shared_renderer_async, 'SetShaderMatrix44', [
		('void', ['const hg::GpuShaderVariable &var', 'const hg::Matrix44 &m'], {'proxy': None, 'route': route_lambda('_RendererAsync_SetShaderMatrix44')}),
		('void', ['const std::string &name', 'const hg::Matrix44 &m'], {'proxy': None, 'route': route_lambda('_RendererAsync_SetShaderMatrix44_name')})

	])
	gen.bind_method_overloads(shared_renderer_async, 'SetShaderTexture', [
		('void', ['const hg::GpuShaderVariable &var', 'const std::shared_ptr<hg::Texture> &texture'], {'proxy': None, 'route': route_lambda('_RendererAsync_SetShaderTexture')}),
		('void', ['const std::string &name', 'const std::shared_ptr<hg::Texture> &texture'], {'proxy': None, 'route': route_lambda('_RendererAsync_SetShaderTexture_name')})
	])
...
def bind_render(gen):
	...
	gen.add_base(shared_surface_shader, gen.get_conv('std::shared_ptr<hg::GpuResource>'))
...
static bool _RenderMaterial_GetMatrix4(hg::RenderMaterial *m, const std::string &name, hg::Matrix4 &o) { if (auto v = m->GetValue(name)) {	o.m[0][0] = v->fv[0];  o.m[1][0] = v->fv[1]; o.m[2][0] = v->fv[2];
																																			o.m[0][1] = v->fv[4];  o.m[1][1] = v->fv[5]; o.m[2][1] = v->fv[6];
																																			o.m[0][2] = v->fv[8];  o.m[1][2] = v->fv[9]; o.m[2][2] = v->fv[10];
																																			o.m[0][3] = v->fv[12];  o.m[1][3] = v->fv[13]; o.m[2][3] = v->fv[14]; return true; } return false; }
																																			
static bool _RenderMaterial_GetMatrix44(hg::RenderMaterial *m, const std::string &name, hg::Matrix44 &o) { if (auto v = m->GetValue(name)) {o.m[0][0] = v->fv[0];  o.m[1][0] = v->fv[1]; o.m[2][0] = v->fv[2]; o.m[3][0] = v->fv[4];
																																			o.m[0][1] = v->fv[4];  o.m[1][1] = v->fv[5]; o.m[2][1] = v->fv[6]; o.m[3][1] = v->fv[7];
																																			o.m[0][2] = v->fv[8];  o.m[1][2] = v->fv[9]; o.m[2][2] = v->fv[10]; o.m[3][2] = v->fv[11];
																																			o.m[0][3] = v->fv[12];  o.m[1][3] = v->fv[13]; o.m[2][3] = v->fv[14]; o.m[3][3] = v->fv[15]; return true; } return false; }
...
static bool _RenderMaterial_SetMatrix4(hg::RenderMaterial *m, const std::string &name, hg::Matrix4 &o) { if (auto v = m->GetValue(name)) {	v->fv[0] = o.m[0][0]; v->fv[1] = o.m[1][0]; v->fv[2] = o.m[2][0]; v->fv[3] = 0;
																																			v->fv[4] = o.m[0][1]; v->fv[5] = o.m[1][1]; v->fv[6] = o.m[2][1]; v->fv[7] = 0;
																																			v->fv[8] = o.m[0][2]; v->fv[9] = o.m[1][2]; v->fv[10] = o.m[2][2]; v->fv[11] = 0;
																																			v->fv[12] = o.m[0][3]; v->fv[13] = o.m[1][3]; v->fv[14] = o.m[2][3]; v->fv[15] = 1; return true; } return false; }

static bool _RenderMaterial_SetMatrix44(hg::RenderMaterial *m, const std::string &name, hg::Matrix44 &o) { if (auto v = m->GetValue(name)) {	v->fv[0] = o.m[0][0]; v->fv[1] = o.m[1][0]; v->fv[2] = o.m[2][0]; v->fv[3] = o.m[3][0];
																																				v->fv[4] = o.m[0][1]; v->fv[5] = o.m[1][1]; v->fv[6] = o.m[2][1]; v->fv[7] = o.m[3][1];
																																				v->fv[8] = o.m[0][2]; v->fv[9] = o.m[1][2]; v->fv[10] = o.m[2][2]; v->fv[11] = o.m[3][2];
																																				v->fv[12] = o.m[0][3]; v->fv[13] = o.m[1][3]; v->fv[14] = o.m[2][3]; v->fv[15] = o.m[3][3]; return true; } return false; }
...
''')
	...
	gen.bind_method(shared_material, 'GetMatrix4', 'bool', ['const std::string &name', 'hg::Matrix4 &o'], {'proxy': None, 'arg_out': ['o'], 'route': lambda args: '_RenderMaterial_GetMatrix4(%s);' % (', '.join(args))})
	gen.bind_method(shared_material, 'GetMatrix44', 'bool', ['const std::string &name', 'hg::Matrix44 &o'], {'proxy': None, 'arg_out': ['o'], 'route': lambda args: '_RenderMaterial_GetMatrix44(%s);' % (', '.join(args))})
	...
	gen.bind_method(shared_material, 'SetMatrix4', 'bool', ['const std::string &name', 'hg::Matrix4 &o'], {'proxy': None, 'route': lambda args: '_RenderMaterial_SetMatrix4(%s);' % (', '.join(args))})	
	gen.bind_method(shared_material, 'SetMatrix44', 'bool', ['const std::string &name', 'hg::Matrix44 &o'], {'proxy': None, 'route': lambda args: '_RenderMaterial_SetMatrix44(%s);' % (', '.join(args))})
...
''')
	gen.bind_method(shared_geometry, 'GetMaterial', 'std::shared_ptr<hg::RenderMaterial>', ['uint32_t index'], {'proxy': None, 'route': route_lambda('_RenderGeometry_GetMaterial'), 'check_rval': check_bool_rval_lambda(gen, 'Invalid material index')})
...
static void _RenderGeometry_SetLodProxyDistance(hg::RenderGeometry *geo, float distance) {
	geo->lod_distance = distance;
}
...
''')
	...
	gen.bind_method(shared_geometry, 'SetLodProxyDistance', 'void', ['float distance'], {'proxy': None, 'route': route_lambda('_RenderGeometry_SetLodProxyDistance')})
	...
	gen.insert_binding_code('''
static std::shared_ptr<hg::RenderGeometry> _RenderGeometry_GetLodProxy(hg::RenderGeometry *geo) { return geo->lod_proxy; }
static float _RenderGeometry_GetLodProxyDistance(hg::RenderGeometry *geo) {	return geo->lod_distance; }
static std::shared_ptr<hg::RenderGeometry> _RenderGeometry_GetShadowProxy(hg::RenderGeometry *geo) { return geo->shadow_proxy; }
''')

	gen.bind_method(shared_geometry, 'GetLodProxy', 'std::shared_ptr<hg::RenderGeometry>', [], {'proxy': None, 'route': route_lambda('_RenderGeometry_GetLodProxy')})
	gen.bind_method(shared_geometry, 'GetLodProxyDistance', 'float', [], {'proxy': None, 'route': route_lambda('_RenderGeometry_GetLodProxyDistance')})
	gen.bind_method(shared_geometry, 'GetShadowProxy', 'std::shared_ptr<hg::RenderGeometry>', [], {'proxy': None, 'route': route_lambda('_RenderGeometry_GetShadowProxy')})
	...
	gen.bind_method(shared_render_system, 'Free', 'void', [], ['proxy'])
	...
	gen.bind_method(shared_render_system, 'CreateShadowMaps', 'void', ['uint16_t resolution'], ['proxy'], bound_name='SetShadowMapsResolution')

	gen.bind_method(shared_render_system, 'PurgeCache', 'size_t', [], ['proxy'])
	gen.bind_method(shared_render_system, 'ClearCache', 'void', [], ['proxy'])
	...
	gen.bind_method(shared_render_system, 'GetMaterial', 'std::shared_ptr<hg::RenderMaterial>', ['const std::string &name'],  {'proxy': None})
	...
	gen.bind_method(shared_render_system, 'GetGeometry', 'std::shared_ptr<hg::RenderGeometry>', ['const std::string &name'], {'proxy': None})
	...
	gen.bind_method(shared_render_system, 'GetSurfaceShader', 'std::shared_ptr<hg::SurfaceShader>', ['const std::string &name'], {'proxy': None})
	...
	gen.bind_method(shared_render_system, 'SetRenderTechnique', 'void', ['hg::RenderTechnique render_technique', '?bool render_to_float_buffers'], ['proxy'])
	...
	gen.bind_method_overloads(shared_render_system, 'SetView', [
		('void', ['const hg::Matrix4 &view', 'const hg::Matrix44 &projection', '?hg::Eye eye'], ['proxy']),
		('void', ['const hg::Matrix4 &view', 'const hg::Matrix4 &previous_view', 'const hg::Matrix44 &projection', '?hg::Eye eye'], ['proxy'])
	])
...
\n''', 'wrapper signatures to cast target language list and std::vector to raw pointers')
	...
	gen.insert_binding_code('''\
static void SetSurfaceShaderInt_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const std::vector<int> &v) { render_system->SetSurfaceShaderInt(shader, name, v.data(), v.size()); }
static void SetSurfaceShaderInt2_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const std::vector<hg::iVector2> &v) { render_system->SetSurfaceShaderInt(shader, name, &v[0].x, v.size()); }

static void SetSurfaceShaderFloat_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const std::vector<float> &v) { render_system->SetSurfaceShaderFloat(shader, name, v.data(), v.size()); }
static void SetSurfaceShaderFloat2_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const std::vector<hg::Vector2> &v) { render_system->SetSurfaceShaderFloat2(shader, name, &v[0].x, v.size()); }
static void SetSurfaceShaderFloat3_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const std::vector<hg::Vector3> &v) { render_system->SetSurfaceShaderFloat3(shader, name, &v[0].x, v.size()); }
static void SetSurfaceShaderFloat4_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const std::vector<hg::Vector4> &v) { render_system->SetSurfaceShaderFloat4(shader, name, &v[0].x, v.size()); }

static void SetSurfaceShaderMatrix3_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const std::vector<hg::Matrix3> &v) { render_system->SetSurfaceShaderMatrix3(shader, name, v.data(), v.size()); }
static void SetSurfaceShaderMatrix4_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const std::vector<hg::Matrix4> &v) { render_system->SetSurfaceShaderMatrix4(shader, name, v.data(), v.size()); }
static void SetSurfaceShaderMatrix44_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const std::vector<hg::Matrix44> &v) { render_system->SetSurfaceShaderMatrix44(shader, name, v.data(), v.size()); }

//
static void SetSurfaceShaderInt_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, int v) { render_system->SetSurfaceShaderInt(shader, name, &v, 1); }
static void SetSurfaceShaderInt2_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const hg::iVector2 &v) { render_system->SetSurfaceShaderInt(shader, name, &v.x, 1); }

static void SetSurfaceShaderFloat_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, float v) { render_system->SetSurfaceShaderFloat(shader, name, &v, 1); }
static void SetSurfaceShaderFloat2_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const hg::Vector2 &v) { render_system->SetSurfaceShaderFloat2(shader, name, &v.x, 1); }
static void SetSurfaceShaderFloat3_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const hg::Vector3 &v) { render_system->SetSurfaceShaderFloat3(shader, name, &v.x, 1); }
static void SetSurfaceShaderFloat4_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const hg::Vector4 &v) { render_system->SetSurfaceShaderFloat4(shader, name, &v.x, 1); }

static void SetSurfaceShaderMatrix3_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const hg::Matrix3 &v) { render_system->SetSurfaceShaderMatrix3(shader, name, &v, 1); }
static void SetSurfaceShaderMatrix4_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const hg::Matrix4 &v) { render_system->SetSurfaceShaderMatrix4(shader, name, &v, 1); }
static void SetSurfaceShaderMatrix44_wrapper(hg::RenderSystem *render_system, hg::SurfaceShader &shader, const std::string &name, const hg::Matrix44 &v) { render_system->SetSurfaceShaderMatrix44(shader, name, &v, 1); }
\n''', 'wrapper signatures to cast target language list and std::vector to raw pointers')

	gen.bind_method_overloads(shared_render_system, 'SetSurfaceShaderInt',
		expand_std_vector_proto(gen, [('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const std::vector<int> &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderInt_wrapper')})]) +
		[('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'int v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderInt_wrapper')})]
	)
	gen.bind_method_overloads(shared_render_system, 'SetSurfaceShaderInt2',
		expand_std_vector_proto(gen, [('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const std::vector<hg::tVector2<int>> &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderInt2_wrapper')})]) +
		[('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const hg::tVector2<int> &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderInt2_wrapper')})]
	)

	gen.bind_method_overloads(shared_render_system, 'SetSurfaceShaderFloat',
		expand_std_vector_proto(gen, [('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const std::vector<float> &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderFloat_wrapper')})]) +
		[('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'float v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderFloat_wrapper')})]
	)
	gen.bind_method_overloads(shared_render_system, 'SetSurfaceShaderFloat2',
		expand_std_vector_proto(gen, [('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const std::vector<hg::tVector2<float>> &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderFloat2_wrapper')})]) +
		[('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const hg::tVector2<float> &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderFloat2_wrapper')})]
	)
	gen.bind_method_overloads(shared_render_system, 'SetSurfaceShaderFloat3',
		expand_std_vector_proto(gen, [('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const std::vector<hg::Vector3> &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderFloat3_wrapper')})]) +
		[('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const hg::Vector3 &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderFloat3_wrapper')})]
	)
	gen.bind_method_overloads(shared_render_system, 'SetSurfaceShaderFloat4',
		expand_std_vector_proto(gen, [('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const std::vector<hg::Vector4> &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderFloat4_wrapper')})]) +
		[('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const hg::Vector4 &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderFloat4_wrapper')})]
	)

	gen.bind_method_overloads(shared_render_system, 'SetSurfaceShaderMatrix3',
		expand_std_vector_proto(gen, [('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const std::vector<hg::Matrix3> &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderMatrix3_wrapper')})]) +
		[('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const hg::Matrix3 &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderMatrix3_wrapper')})]
	)
	gen.bind_method_overloads(shared_render_system, 'SetSurfaceShaderMatrix4',
		expand_std_vector_proto(gen, [('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const std::vector<hg::Matrix4> &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderMatrix4_wrapper')})]) +
		[('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const hg::Matrix4 &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderMatrix4_wrapper')})]
	)
	gen.bind_method_overloads(shared_render_system, 'SetSurfaceShaderMatrix44',
		expand_std_vector_proto(gen, [('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const std::vector<hg::Matrix44> &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderMatrix44_wrapper')})]) +
		[('void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const hg::Matrix44 &v'], {'proxy': None, 'route': route_lambda('SetSurfaceShaderMatrix44_wrapper')})]
	)

	gen.bind_method(shared_render_system, 'SetSurfaceShaderTexture', 'void', ['hg::SurfaceShader &shader', 'const std::string &name', 'const std::shared_ptr<hg::Texture> &t'], ['proxy'])
	...
	gen.bind_method(shared_render_system_async, 'SetRenderTechnique', 'void', ['hg::RenderTechnique technique', '?bool render_to_float_buffers'], ['proxy'])
	...
	gen.bind_method(shared_render_system_async, 'CreateShadowMaps', 'void', ['uint16_t resolution'], ['proxy'], bound_name='SetShadowMapsResolution')

	gen.bind_method_overloads(shared_render_system_async, 'SetView', [
		('void', ['const hg::Matrix4 &view', 'const hg::Matrix44 &projection', '?hg::Eye eye'], ['proxy']),
		('void', ['const hg::Matrix4 &view', 'const hg::Matrix4 &previous_view', 'const hg::Matrix44 &projection', '?hg::Eye eye'], ['proxy'])
	])

	gen.bind_method(shared_render_system_async, 'PurgeCache', 'std::future<size_t>', [], ['proxy'])
	gen.bind_method(shared_render_system_async, 'ClearCache', 'std::future<void>', [], ['proxy'])
	...
	gen.bind_method(shared_render_system_async, 'GetMaterial', 'std::shared_ptr<hg::RenderMaterial>', ['const std::string &name'], {'proxy': None})
	...
	gen.bind_method(shared_render_system_async, 'GetGeometry', 'std::shared_ptr<hg::RenderGeometry>', ['const std::string &name'], {'proxy': None})
	...
	gen.bind_method(shared_render_system_async, 'GetSurfaceShader', 'std::shared_ptr<hg::SurfaceShader>', ['const std::string &name'], {'proxy': None})
	...
	gen.insert_binding_code('''\
static void RenderSystemAsyncDrawLine_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos) { render_system_async->DrawLine(count, pos); }
static void RenderSystemAsyncDrawLine_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col) { render_system_async->DrawLine(count, pos, &col); }
static void RenderSystemAsyncDrawLine_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col, const std::vector<hg::tVector2<float>> &uv) { render_system_async->DrawLine(count, pos, &col, &uv); }

static void RenderSystemAsyncDrawTriangle_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos) { render_system_async->DrawTriangle(count, pos); }
static void RenderSystemAsyncDrawTriangle_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col) { render_system_async->DrawTriangle(count, pos, &col); }
static void RenderSystemAsyncDrawTriangle_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col, const std::vector<hg::tVector2<float>> &uv) { render_system_async->DrawTriangle(count, pos, &col, &uv); }

static void RenderSystemAsyncDrawSprite_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos) { render_system_async->DrawSprite(count, pos); }
static void RenderSystemAsyncDrawSprite_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col) { render_system_async->DrawSprite(count, pos, &col); }
static void RenderSystemAsyncDrawSprite_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col, const std::vector<float> &size) { render_system_async->DrawSprite(count, pos, &col, &size); }
static void RenderSystemAsyncDrawSprite_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col, const std::vector<float> &size, float global_size) { render_system_async->DrawSprite(count, pos, &col, &size, global_size); }
\n''', 'wrapper signatures to cast target language list and std::vector to raw pointers')

	DrawLineAsync_wrapper_route = lambda args: 'RenderSystemAsyncDrawLine_wrapper(%s);' % (', '.join(args))
	DrawTriangleAsync_wrapper_route = lambda args: 'RenderSystemAsyncDrawTriangle_wrapper(%s);' % (', '.join(args))
	DrawSpriteAsync_wrapper_route = lambda args: 'RenderSystemAsyncDrawSprite_wrapper(%s);' % (', '.join(args))

	draw_line_async_protos = [
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos'], {'proxy': None, 'route': DrawLineAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col'], {'proxy': None, 'route': DrawLineAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col', 'const std::vector<hg::tVector2<float>> &uv'], {'proxy': None, 'route': DrawLineAsync_wrapper_route}) ]
	gen.bind_method_overloads(shared_render_system_async, 'DrawLine', expand_std_vector_proto(gen, draw_line_async_protos))

	draw_triangle_async_protos = [
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos'], {'proxy': None, 'route': DrawTriangleAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col'], {'proxy': None, 'route': DrawTriangleAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col', 'const std::vector<hg::tVector2<float>> &uv'], {'proxy': None, 'route': DrawTriangleAsync_wrapper_route}) ]
	gen.bind_method_overloads(shared_render_system_async, 'DrawTriangle', expand_std_vector_proto(gen, draw_triangle_async_protos))

	draw_sprite_async_protos = [
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos'], {'proxy': None, 'route': DrawSpriteAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col'], {'proxy': None, 'route': DrawSpriteAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col', 'const std::vector<float> &size'], {'proxy': None, 'route': DrawSpriteAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col', 'const std::vector<float> &size', 'float global_size'], {'proxy': None, 'route': DrawSpriteAsync_wrapper_route}) ]
	gen.bind_method_overloads(shared_render_system_async, 'DrawSprite', expand_std_vector_proto(gen, draw_sprite_async_protos))
	
	gen.insert_binding_code('''\
static void RenderSystemAsyncDrawLineAuto_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos) { render_system_async->DrawLineAuto(count, pos); }
static void RenderSystemAsyncDrawLineAuto_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col) { render_system_async->DrawLineAuto(count, pos, &col); }
static void RenderSystemAsyncDrawLineAuto_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col, const std::vector<hg::tVector2<float>> &uv, std::shared_ptr<hg::Texture> texture) { render_system_async->DrawLineAuto(count, pos, &col, &uv, texture); }

static void RenderSystemAsyncDrawTriangleAuto_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos) { render_system_async->DrawTriangleAuto(count, pos); }
static void RenderSystemAsyncDrawTriangleAuto_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col) { render_system_async->DrawTriangleAuto(count, pos, &col); }
static void RenderSystemAsyncDrawTriangleAuto_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col, const std::vector<hg::tVector2<float>> &uv, std::shared_ptr<hg::Texture> texture) { render_system_async->DrawTriangleAuto(count, pos, &col, &uv, texture); }

static void RenderSystemAsyncDrawSpriteAuto_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos) { render_system_async->DrawSpriteAuto(count, pos); }
static void RenderSystemAsyncDrawSpriteAuto_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col) { render_system_async->DrawSpriteAuto(count, pos, &col); }
static void RenderSystemAsyncDrawSpriteAuto_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col, const std::vector<float> &size) { render_system_async->DrawSpriteAuto(count, pos, &col, &size); }
static void RenderSystemAsyncDrawSpriteAuto_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col, const std::vector<float> &size, std::shared_ptr<hg::Texture> texture) { render_system_async->DrawSpriteAuto(count, pos, &col, &size, texture); }
static void RenderSystemAsyncDrawSpriteAuto_wrapper(hg::RenderSystemAsync *render_system_async, uint32_t count, const std::vector<hg::Vector3> &pos, const std::vector<hg::Color> &col, const std::vector<float> &size, std::shared_ptr<hg::Texture> texture, float global_size) { render_system_async->DrawSpriteAuto(count, pos, &col, &size, texture, global_size); }
\n''', 'wrapper signatures to cast target language list and std::vector to raw pointers')

	DrawLineAutoAsync_wrapper_route = lambda args: 'RenderSystemAsyncDrawLineAuto_wrapper(%s);' % (', '.join(args))
	DrawTriangleAutoAsync_wrapper_route = lambda args: 'RenderSystemAsyncDrawTriangleAuto_wrapper(%s);' % (', '.join(args))
	DrawSpriteAutoAsync_wrapper_route = lambda args: 'RenderSystemAsyncDrawSpriteAuto_wrapper(%s);' % (', '.join(args))

	draw_line_async_auto_protos = [
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos'], {'proxy': None, 'route': DrawLineAutoAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col'], {'proxy': None, 'route': DrawLineAutoAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col', 'const std::vector<hg::tVector2<float>> &uv', 'std::shared_ptr<hg::Texture> texture'], {'proxy': None, 'route': DrawLineAutoAsync_wrapper_route}) ]
	gen.bind_method_overloads(shared_render_system_async, 'DrawLineAuto', expand_std_vector_proto(gen, draw_line_async_auto_protos))

	draw_triangle_async_auto_protos = [
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos'], {'proxy': None, 'route': DrawTriangleAutoAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col'], {'proxy': None, 'route': DrawTriangleAutoAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col', 'const std::vector<hg::tVector2<float>> &uv', 'std::shared_ptr<hg::Texture> texture'], {'proxy': None, 'route': DrawTriangleAutoAsync_wrapper_route}) ]
	gen.bind_method_overloads(shared_render_system_async, 'DrawTriangleAuto', expand_std_vector_proto(gen, draw_triangle_async_auto_protos))

	draw_sprite_async_auto_protos = [
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos'], {'proxy': None, 'route': DrawSpriteAutoAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col'], {'proxy': None, 'route': DrawSpriteAutoAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col', 'const std::vector<float> &size'], {'proxy': None, 'route': DrawSpriteAutoAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col', 'const std::vector<float> &size', 'std::shared_ptr<hg::Texture> texture'], {'proxy': None, 'route': DrawSpriteAutoAsync_wrapper_route}),
		('void', ['uint32_t count', 'const std::vector<hg::Vector3> &pos', 'const std::vector<hg::Color> &col', 'const std::vector<float> &size', 'std::shared_ptr<hg::Texture> texture', 'float global_size'], {'proxy': None, 'route': DrawSpriteAutoAsync_wrapper_route}) ]
	gen.bind_method_overloads(shared_render_system_async, 'DrawSpriteAuto', expand_std_vector_proto(gen, draw_sprite_async_auto_protos))

	gen.bind_method(shared_render_system_async, 'DrawQuad2D', 'void', ['const hg::Rect<float> &src_rect', 'const hg::Rect<float> &dst_rect'], ['proxy'])
	gen.bind_method(shared_render_system_async, 'DrawFullscreenQuad', 'void', ['const hg::tVector2<float> &uv'], ['proxy'])

	gen.bind_method(shared_render_system_async, 'SetSurfaceShaderInt', 'void', ['const std::shared_ptr<hg::SurfaceShader> &shader', 'const std::string &name', 'const std::vector<int> &v'], ['proxy'])
	gen.bind_method(shared_render_system_async, 'SetSurfaceShaderInt2', 'void', ['const std::shared_ptr<hg::SurfaceShader> &shader', 'const std::string &name', 'const std::vector<hg::tVector2<int>> &v'], ['proxy'])

	gen.bind_method(shared_render_system_async, 'SetSurfaceShaderFloat', 'void', ['const std::shared_ptr<hg::SurfaceShader> &shader', 'const std::string &name', 'const std::vector<float> &v'], ['proxy'])
	gen.bind_method(shared_render_system_async, 'SetSurfaceShaderFloat2', 'void', ['const std::shared_ptr<hg::SurfaceShader> &shader', 'const std::string &name', 'const std::vector<hg::tVector2<float>> &v'], ['proxy'])
	gen.bind_method(shared_render_system_async, 'SetSurfaceShaderFloat3', 'void', ['const std::shared_ptr<hg::SurfaceShader> &shader', 'const std::string &name', 'const std::vector<hg::Vector3> &v'], ['proxy'])
	gen.bind_method(shared_render_system_async, 'SetSurfaceShaderFloat4', 'void', ['const std::shared_ptr<hg::SurfaceShader> &shader', 'const std::string &name', 'const std::vector<hg::Vector4> &v'], ['proxy'])

	gen.bind_method(shared_render_system_async, 'SetSurfaceShaderMatrix3', 'void', ['const std::shared_ptr<hg::SurfaceShader> &shader', 'const std::string &name', 'const std::vector<hg::Matrix3> &v'], ['proxy'])
	gen.bind_method(shared_render_system_async, 'SetSurfaceShaderMatrix4', 'void', ['const std::shared_ptr<hg::SurfaceShader> &shader', 'const std::string &name', 'const std::vector<hg::Matrix4> &v'], ['proxy'])
	gen.bind_method(shared_render_system_async, 'SetSurfaceShaderMatrix44', 'void', ['const std::shared_ptr<hg::SurfaceShader> &shader', 'const std::string &name', 'const std::vector<hg::Matrix44> &v'], ['proxy'])

	gen.bind_method(shared_render_system_async, 'SetSurfaceShaderTexture', 'void', ['const std::shared_ptr<hg::SurfaceShader> &shader', 'const std::string &name', 'const std::shared_ptr<hg::Texture> &t'], ['proxy'])
...
def bind_plus(gen):
	...
	gen.bind_constructor(window_conv, [])
...
''')
	...
	gen.bind_method(plus_conv, 'WaitForRender', 'void', [])
	...
	gen.insert_binding_code('''
static void _Plus_Lines2D(hg::Plus *plus, const std::vector<hg::tVector2<float>> &points, const std::vector<hg::Color> &colors) {
	if (points.size() != colors.size()) {
		hg::debug("Mismatch between the number of points and colors");
		return;
	}
	...
	for (size_t i = 1; i < count; ++i) {
		auto &p_a = points[i - 1], &p_b = points[i];
		auto &c_a = colors[i - 1], &c_b = colors[i];
		plus->Line2D(p_a.x, p_a.y, p_b.x, p_b.y, c_a, c_b);
	}
}
''')
	gen.bind_method(plus_conv, 'Lines2D', 'void', ['const std::vector<hg::tVector2<float>> & points', 'const std::vector<hg::Color> & colors'], { 'route': lambda args: '_Plus_Lines2D(%s);' % (', '.join(args))})

	gen.insert_binding_code('''
static void _Plus_Quads2D(hg::Plus *plus, const std::vector<hg::tVector2<float>> &points, const float &size, const hg::Color &color, std::shared_ptr<hg::Texture> texture) {
	for (auto &p : points)
		plus->Quad2D(p.x - size, p.y - size, p.x - size, p.y + size, p.x + size, p.y + size, p.x + size, p.y - size, color, color, color, color, texture);
}
''')
	gen.bind_method(plus_conv, 'Quads2D', 'void', ['const std::vector<hg::tVector2<float>> & points', 'const float &size', 'const hg::Color &color', 'std::shared_ptr<hg::Texture> texture'], { 'route': lambda args: '_Plus_Quads2D(%s);' % (', '.join(args))})
	...
	gen.bind_method(plus_conv, 'RenderScene', 'void', ['hg::Scene &scene'], [])
...
def bind_picture(gen):
	...
	gen.insert_binding_code('''
static intptr_t _SharedPicture_GetDataPointer(hg::Picture *picture) {
	return reinterpret_cast<intptr_t>(picture->GetData());
}
''')	
	gen.bind_method(shared_picture, 'GetDataPointer', 'intptr_t', [], {'proxy':None, 'route': route_lambda('_SharedPicture_GetDataPointer')})
...
''')
	...
	gen.insert_binding_code('''
static bool SavePicture(std::shared_ptr<hg::Picture> &picture, const std::string &path, const std::string &codec_name, const std::string &parm = std::string()) {
	return hg::g_picture_io.get().Save(*picture, path, codec_name, parm);
}
''')
	...
	gen.insert_binding_code('static std::vector<std::string> GetSupportedPictureExt() { return hg::g_picture_io.get().GetSupportedExt(); }')
	gen.bind_function('GetSupportedPictureExt', 'std::vector<std::string>', [])
...
def bind_document(gen):
	...
	gen.bind_named_enum('hg::DocumentReader::ValueType', ['Null', 'Bool', 'Int32', 'UInt32', 'Float', 'String'], prefix='DocumentValue')
	...
	gen.bind_method(shared_doc_reader, 'GetFirstKey', 'bool', ['const std::string &key', 'hg::DocumentReader::ValueType &type'], {'proxy': None, 'arg_out': ['key', 'type']})
	gen.bind_method(shared_doc_reader, 'GetNextKey', 'bool', ['const std::string &key', 'hg::DocumentReader::ValueType &type'], {'proxy': None, 'arg_out': ['key', 'type']})
...
def bind_movie(gen):
	...
	gen.bind_method_overloads(shared_movie, 'Open', [
		('bool', ['std::shared_ptr<hg::Renderer> renderer', 'std::shared_ptr<hg::Mixer> mixer', 'const std::string &filename', '?bool paused'], ['proxy']),
		('bool', ['std::shared_ptr<hg::Renderer> renderer', 'const std::string &filename', '?bool paused'], ['proxy'])
	])
...
def bind_imgui(gen):
	...
	gen.bind_method(imdrawlist, 'PushClipRect', 'void', ['hg::tVector2<float> clip_rect_min', 'hg::tVector2<float> clip_rect_min', '?bool intersect_with_curent_clip_rect'])
	gen.bind_method(imdrawlist, 'PushClipRectFullScreen', 'void', [])
	gen.bind_method(imdrawlist, 'PopClipRect', 'void', [])
	gen.bind_method(imdrawlist, 'PushTextureID', 'void', ['hg::Texture *texture'])
	gen.bind_method(imdrawlist, 'PopTextureID', 'void', [])
	gen.bind_method(imdrawlist, 'GetClipRectMin', 'hg::tVector2<float>', [])
	gen.bind_method(imdrawlist, 'GetClipRectMax', 'hg::tVector2<float>', [])

	gen.bind_method(imdrawlist, 'AddLine', 'void', ['const hg::tVector2<float> &a', 'const hg::tVector2<float> &b', 'ImU32 col', '?float thickness'])
	gen.bind_method(imdrawlist, 'AddRect', 'void', ['const hg::tVector2<float> &a', 'const hg::tVector2<float> &b', 'ImU32 col', '?float rounding', '?int rounding_corner_flags', '?float thickness'])
	gen.bind_method(imdrawlist, 'AddRectFilled', 'void', ['const hg::tVector2<float> &a', 'const hg::tVector2<float> &b', 'ImU32 col', '?float rounding', '?int rounding_corner_flags'])
	gen.bind_method(imdrawlist, 'AddRectFilledMultiColor', 'void', ['const hg::tVector2<float> &a', 'const hg::tVector2<float> &b', 'ImU32 col_upr_left', 'ImU32 col_upr_right', 'ImU32 col_bot_right', 'ImU32 col_bot_left'])
	gen.bind_method(imdrawlist, 'AddQuad', 'void', ['const hg::tVector2<float> &a', 'const hg::tVector2<float> &b', 'const hg::tVector2<float> &c', 'const hg::tVector2<float> &d', 'ImU32 col', '?float thickness'])
	gen.bind_method(imdrawlist, 'AddQuadFilled', 'void', ['const hg::tVector2<float> &a', 'const hg::tVector2<float> &b', 'const hg::tVector2<float> &c', 'const hg::tVector2<float> &d', 'ImU32 col'])
	gen.bind_method(imdrawlist, 'AddTriangle', 'void', ['const hg::tVector2<float> &a', 'const hg::tVector2<float> &b', 'const hg::tVector2<float> &c', 'ImU32 col', '?float thickness'])
	gen.bind_method(imdrawlist, 'AddTriangleFilled', 'void', ['const hg::tVector2<float> &a', 'const hg::tVector2<float> &b', 'const hg::tVector2<float> &c', 'ImU32 col'])
	gen.bind_method(imdrawlist, 'AddCircle', 'void', ['const hg::tVector2<float> &centre', 'float radius', 'ImU32 col', '?int num_segments', '?float thickness'])
	gen.bind_method(imdrawlist, 'AddCircleFilled', 'void', ['const hg::tVector2<float> &centre', 'float radius', 'ImU32 col', '?int num_segments'])

	gen.insert_binding_code('''static void _ImDrawList_AddText(ImDrawList *self, const ImFont *font, float font_size, const hg::tVector2<float> &pos, ImU32 col, const char *text, float wrap_width = 0.0f, const hg::Vector4 *cpu_fine_clip_rect = nullptr) {
	...
	gen.bind_method_overloads(imdrawlist, 'AddText', [
		('void', ['const hg::tVector2<float> &pos', 'ImU32 col', 'const char *text'], []),
		('void', ['const ImFont *font', 'float font_size', 'const hg::tVector2<float> &pos', 'ImU32 col', 'const char *text', '?float wrap_width', '?const hg::Vector4 *cpu_fine_clip_rect'], {'route': route_lambda('_ImDrawList_AddText')})
	])

	gen.bind_method_overloads(imdrawlist, 'AddImage', [
		('void', ['hg::Texture *texture', 'const hg::tVector2<float> &a', 'const hg::tVector2<float> &b'], []),
		('void', ['hg::Texture *texture', 'const hg::tVector2<float> &a', 'const hg::tVector2<float> &b', 'const hg::tVector2<float> &uv_a', 'const hg::tVector2<float> &uv_b', '?ImU32 col'], [])
	])
	gen.bind_method_overloads(imdrawlist, 'AddImageQuad', [
		('void', ['hg::Texture *texture', 'const hg::tVector2<float> &a', 'const hg::tVector2<float> &b', 'const hg::tVector2<float> &c', 'const hg::tVector2<float> &d'], []),
		('void', ['hg::Texture *texture', 'const hg::tVector2<float> &a', 'const hg::tVector2<float> &b', 'const hg::tVector2<float> &c', 'const hg::tVector2<float> &d', 'const hg::tVector2<float> &uv_a', 'const hg::tVector2<float> &uv_b', 'const hg::tVector2<float> &uv_c', 'const hg::tVector2<float> &uv_d', '?ImU32 col'], [])
	])
	gen.bind_method(imdrawlist, 'AddImageRounded', 'void', ['hg::Texture *texture', 'const hg::tVector2<float> &a', 'const hg::tVector2<float> &b', 'const hg::tVector2<float> &uv_a', 'const hg::tVector2<float> &uv_b', 'ImU32 col', 'float rounding', '?ImDrawCornerFlags rounding_corners'])

	gen.insert_binding_code('''
static void _ImDrawList_AddPolyline(ImDrawList *self, const std::vector<hg::tVector2<float>> &points, ImU32 col, bool closed, float thickness) { self->AddPolyline(reinterpret_cast<const ImVec2 *>(points.data()), points.size(), col, closed, thickness); }
static void _ImDrawList_AddConvexPolyFilled(ImDrawList *self, const std::vector<hg::tVector2<float>> &points, ImU32 col) { self->AddConvexPolyFilled(reinterpret_cast<const ImVec2 *>(points.data()), points.size(), col); }
''')
	gen.bind_method(imdrawlist, 'AddPolyline', 'void', ['const std::vector<hg::tVector2<float>> &points', 'ImU32 col', 'bool closed', 'float thickness'], {'route': route_lambda('_ImDrawList_AddPolyline')})
	gen.bind_method(imdrawlist, 'AddConvexPolyFilled', 'void', ['const std::vector<hg::tVector2<float>> &points', 'ImU32 col'], {'route': route_lambda('_ImDrawList_AddConvexPolyFilled')})
	gen.bind_method(imdrawlist, 'AddBezierCurve', 'void', ['const hg::tVector2<float> &pos0', 'const hg::tVector2<float> &cp0', 'const hg::tVector2<float> &cp1', 'const hg::tVector2<float> &pos1', 'ImU32 col', 'float thickness', '?int num_segments'])

	gen.bind_method(imdrawlist, 'PathClear', 'void', [])

	gen.bind_method(imdrawlist, 'PathLineTo', 'void', ['const hg::tVector2<float> &pos'])
	gen.bind_method(imdrawlist, 'PathLineToMergeDuplicate', 'void', ['const hg::tVector2<float> &pos'])
	...
	gen.bind_method(imdrawlist, 'PathArcTo', 'void', ['const hg::tVector2<float> &centre', 'float radius', 'float a_min', 'float a_max', '?int num_segments'])
	gen.bind_method(imdrawlist, 'PathArcToFast', 'void', ['const hg::tVector2<float> &centre', 'float radius', 'int a_min_of_12', 'int a_max_of_12'])
	gen.bind_method(imdrawlist, 'PathBezierCurveTo', 'void', ['const hg::tVector2<float> &p1', 'const hg::tVector2<float> &p2', 'const hg::tVector2<float> &p3', '?int num_segments'])
	gen.bind_method(imdrawlist, 'PathRect', 'void', ['const hg::tVector2<float> &rect_min', 'const hg::tVector2<float> &rect_max', '?float rounding', '?ImDrawCornerFlags rounding_corners_flags'])
	...
	gen.bind_function('ImGui::BeginChild', 'bool', ['const char *id', '?const hg::tVector2<float> &size', '?bool border', '?ImGuiWindowFlags extra_flags'], bound_name='ImGuiBeginChild')
	...
	gen.insert_binding_code('''
static void _ImGuiSetWindowGlobalFontScale(float scale) { ImGui::GetIO().FontGlobalScale = scale; }
''')
	gen.bind_function('_ImGuiSetWindowGlobalFontScale', 'void', ['float scale'], bound_name='ImGuiSetWindowGlobalFontScale')
	...
	gen.bind_function('ImGui::SetNextWindowSizeConstraints', 'void', ['const hg::tVector2<float> &size_min', 'const hg::tVector2<float> &size_max'], bound_name='ImGuiSetNextWindowSizeConstraints')
...
static bool _ImGuiColorButton(const char *id, hg::Color &color, ImGuiColorEditFlags flags = 0, const hg::tVector2<float> &size = hg::tVector2<float>(0, 0)) { return ImGui::ColorButton(id, *(hg::Vector4 *)&color, flags, size); }
...
''')
	...
	#IMGUI_API bool InputTextMultiline(const char* label, char* buf, size_t buf_size, const hg::tVector2<float>& size = hg::tVector2<float>(0,0), ImGuiInputTextFlags flags = 0, ImGuiTextEditCallback callback = NULL, void* user_data = NULL);
...
static bool _ImGuiInputFloat(const char *label, float &v, float step = 0.f, float step_fast = 0.f, int decimal_precision = 4, ImGuiInputTextFlags extra_flags = 0) { return ImGui::InputFloat(label, &v, step, step_fast, hg::format("%.%1f").arg(decimal_precision).c_str(), extra_flags); }
static bool _ImGuiInputVector2(const char *label, hg::tVector2<float> &v, int decimal_precision = 4, ImGuiInputTextFlags extra_flags = 0) { return ImGui::InputFloat2(label, &v.x, hg::format("%.%1f").arg(decimal_precision).c_str(), extra_flags); }
static bool _ImGuiInputVector3(const char *label, hg::Vector3 &v, int decimal_precision = 4, ImGuiInputTextFlags extra_flags = 0) { return ImGui::InputFloat3(label, &v.x, hg::format("%.%1f").arg(decimal_precision).c_str(), extra_flags); }
static bool _ImGuiInputVector4(const char *label, hg::Vector4 &v, int decimal_precision = 4, ImGuiInputTextFlags extra_flags = 0) { return ImGui::InputFloat4(label, &v.x, hg::format("%.%1f").arg(decimal_precision).c_str(), extra_flags); }
''')

	gen.bind_function_overloads('ImGui::InputInt', [
		('bool', ['const char *label', 'int *v'], {'arg_in_out': ['v']}),
		('bool', ['const char *label', 'int *v', 'int step', 'int step_fast', '?ImGuiInputTextFlags extra_flags'], {'arg_in_out': ['v']})
	...
	gen.bind_function_overloads('_ImGuiInputiVector2', [
		('bool', ['const char *label', 'hg::tVector2<int> &v', '?ImGuiInputTextFlags extra_flags'], {'arg_in_out': ['v']})
	], bound_name='ImGuiInputIntVector2')
...
static bool _ImGuiSlideriVector2(const char *label, hg::tVector2<int> &v, int v_min, int v_max, const char *format="%.0f") { return ImGui::SliderInt2(label, &v.x, v_min, v_max, format); }
...
static bool _ImGuiSliderVector2(const char *label, hg::tVector2<float> &v, float v_min, float v_max, const char *format="%.3f") { return ImGui::SliderFloat2(label, &v.x, v_min, v_max, format); }
static bool _ImGuiSliderVector3(const char *label, hg::Vector3 &v, float v_min, float v_max, const char *format="%.3f") { return ImGui::SliderFloat3(label, &v.x, v_min, v_max, format); }
static bool _ImGuiSliderVector4(const char *label, hg::Vector4 &v, float v_min, float v_max, const char *format="%.3f") { return ImGui::SliderFloat4(label, &v.x, v_min, v_max, format); }
''')
	...
	gen.bind_function_overloads('_ImGuiSlideriVector2', [
		('bool', ['const char *label', 'hg::tVector2<int> &v', 'int v_min', 'int v_max', '?const char *format'], {'arg_in_out': ['v']})
	], bound_name='ImGuiSliderIntVector2')
	...
	gen.bind_function_overloads('_ImGuiSliderVector2', [
        ('bool', ['const char *label', 'hg::tVector2<float> &v', 'float v_min', 'float v_max', '?const char *format'], {'arg_in_out': ['v']}),
    ], bound_name='ImGuiSliderVector2')
	gen.bind_function_overloads('_ImGuiSliderVector3', [
        ('bool', ['const char *label', 'hg::Vector3 &v', 'float v_min', 'float v_max', '?const char *format'], {'arg_in_out': ['v']}),
    ], bound_name='ImGuiSliderVector3')
	gen.bind_function_overloads('_ImGuiSliderVector4', [
        ('bool', ['const char *label', 'hg::Vector4 &v', 'float v_min', 'float v_max', '?const char *format'], {'arg_in_out': ['v']}),
    ], bound_name='ImGuiSliderVector4')
	...
	gen.insert_binding_code('''\
static bool _ImGuiSelectable(const char *label, bool selected = false, ImGuiSelectableFlags flags = 0, const hg::tVector2<float> &size = hg::tVector2<float>(0.f, 0.f)) { return ImGui::Selectable(label, selected, flags, hg::tVector2<float>(size)); }
...
static void _ImGuiPushClipRect(const hg::tVector2<float> &clip_rect_min, const hg::tVector2<float> &clip_rect_max, bool intersect_with_current_clip_rect) {
	ImGui::PushClipRect(hg::tVector2<float>(clip_rect_min), hg::tVector2<float>(clip_rect_max), intersect_with_current_clip_rect);
}
...
''')
	...
	#IMGUI_API bool          BeginChildFrame(ImGuiID id, const hg::tVector2<float>& size, ImGuiWindowFlags extra_flags = 0);	// helper to create a child window / scrolling region that looks like a normal widget frame
	...
	gen.bind_function('hg::ImGuiAddFont', 'ImFont*', ['hg::RendererAsync &renderer_async', 'const std::string &font_path', 'const float &font_size'], bound_name='ImGuiAddFont')
...
\n''')
	...
	hg::Init();
	InitializePluginsDefaultSearchPath(L);
	hg::InstallCustomPackageLoader(L);
\n''')
	else:
		gen.add_custom_init_code('''\
	hg::Init();
