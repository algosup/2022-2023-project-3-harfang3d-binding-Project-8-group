...
template <size_t WordSize>
void *ToHostEndianness(void *ptr, size_t size, Endianness from_endianness = BigEndian) {
...
template <class T>
T ToHostEndianness(const T &v, Endianness from_endianness = BigEndian) {
	if (GetHostEndianness() == from_endianness)
		return v;
