...
template <typename T>
inline bool asbool(const T &v) { return v ? true : false; }
...
template <typename T>
inline bool set_if_not_equal(T &variable, const T &value) {
...
template <typename T>
shared_ref_<T> shared_ref(const std::shared_ptr<T> &p) { return shared_ref_<T>(p); }
template <typename T>
shared_ref_<T> shared_ref(const std::shared_ptr<T> &&p) { return shared_ref_<T>(p); }

template <typename T, typename D>
shared_ref_<T> shared_ref_cast(const std::shared_ptr<D> &p) { return shared_ref<T>(std::static_pointer_cast<D>(p)); }
...
template <typename T>
std::weak_ptr<T> weak_from_this(std::enable_shared_from_this<T> *s) { return std::weak_ptr<T>(s->shared_from_this()); }
template <typename T>
std::weak_ptr<const T> weak_from_this(const std::enable_shared_from_this<T> *s) { return std::weak_ptr<const T>(s->shared_from_this()); }

// from https://channel9.msdn.com/Events/GoingNative/2013/Cpp-Seasoning
template <typename I>
auto slide(I f, I l, I p) -> std::pair<I, I> { // I model RandomAccessIterator
	if (p < f)
...
template <class CastType, class Type>
CastType numeric_cast(Type v) {
	static_assert(std::is_integral<CastType>::value && std::is_integral<Type>::value, "numeric_cast is only valid for integral types");

#if _DEBUG
	auto mn = std::numeric_limits<CastType>::min(),
		 mx = std::numeric_limits<CastType>::max();
