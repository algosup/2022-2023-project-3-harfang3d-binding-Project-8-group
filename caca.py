...
for match in re.finditer(pattern, content, re.MULTILINE):
    ...
    if return_type.strip() == "" :
      print(f'gen.bind_constructor(class_conv , [{argument_str}])')
    else:
      print(f'gen.bind_function("{function_name}", "{return_type}", [{argument_str}])')
      
# iterate over matches and print the desired format
for match in re.finditer(class_pattern, content, re.MULTILINE):
    print(f"class_conv = gen.begin_class('{match.group(1)}')" )
